...
Run `just check` before submitting a change. It builds and tests the Rust and
Go workspaces, checks shell scripts and GitHub Actions with ShellCheck and
actionlint, verifies formatting, and checks spelling. The default Nix
development shell includes the required `actionlint` executable.

The release workflow is generated by cargo-dist. Edit the hand-written jobs in
`.github/workflow-partials/release-handwritten-jobs.yml`, then run
`just gen-release-workflow`; the generator normalizes cargo-dist's embedded
shell, and the standard checks validate the assembled workflow.
