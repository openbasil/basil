...
pub struct Limits {
    ...
    /// Maximum forwarded calls, with no admission queue. Challenges may occupy
    /// at most one fewer slot, so a value of one admits only invocations.
    pub in_flight: usize,
...
fn prepare_request(
    ...
    let permit = state.admit(route, source)?;
    Ok(PreparedRequest {
        route,
        source,
        content_length,
...
impl AppState {
    fn new(
        ...
        Self {
            ...
            challenge_client,
            invoke_client,
            forwarding: ForwardAdmission::new(limits.in_flight),
            admission: Mutex::new(Admission::new(limits)),
            rejection_log: Mutex::new(TokenBucket::new(10, 20)),
        }
    }

    fn admit(&self, route: Route, source: IpAddr) -> Result<ForwardPermit, Problem> {
        admit_forwarding(&self.forwarding, &self.admission, route, source)
    }
...
fn admit_forwarding(
    ...
    route: Route,
    source: IpAddr,
) -> Result<ForwardPermit, Problem> {
    let permit = forwarding.try_acquire(route)?;
    admission
        .lock()
        .map_err(|_| Problem::internal())?
        .admit(route, source)?;
...
struct ForwardAdmission {
    total: Arc<Semaphore>,
    challenges: Arc<Semaphore>,
}

impl ForwardAdmission {
    fn new(total: usize) -> Self {
        Self {
            total: Arc::new(Semaphore::new(total)),
            challenges: Arc::new(Semaphore::new(total.saturating_sub(1))),
        }
    }

    fn try_acquire(&self, route: Route) -> Result<ForwardPermit, Problem> {
        let challenge = match route {
            Route::Challenge => Some(
...
struct ForwardPermit {
    ...
    _challenge: Option<OwnedSemaphorePermit>,
}
...
impl Admission {
    ...
    fn admit(&mut self, route: Route, source: IpAddr) -> Result<(), Problem> {
        let now = Instant::now();
        let reserve = match route {
            Route::Challenge => 1,
            Route::Invoke => 0,
        };
        let source_exists = self.sources.contains_key(&source);
        if !source_exists {
            let available_source_slots = match route {
                Route::Challenge => self.maximum_sources.saturating_sub(1),
                Route::Invoke => self.maximum_sources,
            };
            if self.sources.len() >= available_source_slots {
                return Err(Problem::overloaded());
            }
        }

        let new_source_bucket =
            TokenBucket::new_at(self.per_source_rate, self.per_source_burst, now);
        let source_bucket = self.sources.get(&source).unwrap_or(&new_source_bucket);
        if !source_bucket.available(now, reserve) || !self.global.available(now, reserve) {
            return Err(Problem::overloaded());
        }

        self.global.consume(now);
        if let Some(source_bucket) = self.sources.get_mut(&source) {
            source_bucket.consume(now);
        } else {
            let mut source_bucket = new_source_bucket;
            source_bucket.consume(now);
            self.sources.insert(source, source_bucket);
        }
        Ok(())
    }
}
...
impl TokenBucket {
    ...
    fn new_at(rate: u32, burst: u32, now: Instant) -> Self {
        Self {
            rate: f64::from(rate),
    ...
    fn available(&self, now: Instant, reserve: u32) -> bool {
        self.refilled_tokens(now) >= f64::from(reserve.saturating_add(1))
    }

    fn refilled_tokens(&self, now: Instant) -> f64 {
    ...
    fn consume(&mut self, now: Instant) {
        self.tokens = self.refilled_tokens(now) - 1.0;
        self.last = now;
    }
}
...
mod tests {
    ...
    fn rate_and_source_table_pressure_fail_without_queueing() {
        ...
        assert!(admission.admit(Route::Invoke, first).is_ok());
        assert_eq!(
            admission.admit(Route::Invoke, first).unwrap_err().code,
            "OVERLOADED"
        );
        ...
        assert!(admission.admit(Route::Invoke, first).is_ok());
        assert_eq!(
            admission.admit(Route::Invoke, second).unwrap_err().code,
            "OVERLOADED"
        );
    ...
    #[test]
    fn challenge_forwarding_preserves_one_invoke_slot_inside_the_total_ceiling() {
        let forwarding = ForwardAdmission::new(2);
        let challenge = forwarding.try_acquire(Route::Challenge).unwrap();
        assert!(forwarding.try_acquire(Route::Challenge).is_err());
        let invocation = forwarding.try_acquire(Route::Invoke).unwrap();
        assert!(forwarding.try_acquire(Route::Invoke).is_err());
        drop(invocation);
        assert!(forwarding.try_acquire(Route::Invoke).is_ok());
        drop(challenge);

        let invoke_only = ForwardAdmission::new(1);
        assert!(invoke_only.try_acquire(Route::Challenge).is_err());
        assert!(invoke_only.try_acquire(Route::Invoke).is_ok());
    }

    #[test]
    fn full_challenge_lane_mutates_no_rate_or_source_state() {
        let forwarding = ForwardAdmission::new(2);
        let _held = forwarding.try_acquire(Route::Challenge).unwrap();
        let configured = limits();
        let admission = Mutex::new(Admission::new(&configured));
        let source: IpAddr = "192.0.2.1".parse().unwrap();
        let before = admission.lock().unwrap();
        let global_tokens = before.global.tokens;
        let global_last = before.global.last;
        drop(before);

        assert_eq!(
            admit_forwarding(&forwarding, &admission, Route::Challenge, source)
                .err()
                .expect("a full challenge lane is overloaded")
                .code,
            "OVERLOADED"
        );
        let after = admission.lock().unwrap();
        assert_eq!(after.global.tokens.to_bits(), global_tokens.to_bits());
        assert_eq!(after.global.last, global_last);
        assert!(after.sources.is_empty());
        drop(after);
    }

    #[test]
    fn challenge_rate_pressure_preserves_global_and_same_source_invoke_tokens() {
        let source: IpAddr = "192.0.2.1".parse().unwrap();
        let mut configured = limits();
        configured.global_rate = 1;
        configured.global_burst = 2;
        configured.per_source_rate = 1;
        configured.per_source_burst = 2;
        let mut admission = Admission::new(&configured);

        assert!(admission.admit(Route::Challenge, source).is_ok());
        let global_tokens = admission.global.tokens;
        let global_last = admission.global.last;
        let source_tokens = admission.sources[&source].tokens;
        let source_last = admission.sources[&source].last;
        assert_eq!(
            admission.admit(Route::Challenge, source).unwrap_err().code,
            "OVERLOADED"
        );
        assert_eq!(admission.global.tokens.to_bits(), global_tokens.to_bits());
        assert_eq!(admission.global.last, global_last);
        assert_eq!(
            admission.sources[&source].tokens.to_bits(),
            source_tokens.to_bits()
        );
        assert_eq!(admission.sources[&source].last, source_last);
        assert!(admission.admit(Route::Invoke, source).is_ok());
    }

    #[test]
    fn challenge_source_pressure_preserves_the_invoke_map_slot() {
        let challenge_source: IpAddr = "192.0.2.1".parse().unwrap();
        let invoke_source: IpAddr = "192.0.2.2".parse().unwrap();
        let mut configured = limits();
        configured.global_burst = 10;
        configured.per_source_burst = 10;
        configured.source_buckets = 2;
        let mut admission = Admission::new(&configured);

        assert!(admission.admit(Route::Challenge, challenge_source).is_ok());
        let global_tokens = admission.global.tokens;
        let global_last = admission.global.last;
        assert_eq!(
            admission
                .admit(Route::Challenge, invoke_source)
                .unwrap_err()
                .code,
            "OVERLOADED"
        );
        assert_eq!(admission.global.tokens.to_bits(), global_tokens.to_bits());
        assert_eq!(admission.global.last, global_last);
        assert!(!admission.sources.contains_key(&invoke_source));
        assert!(admission.admit(Route::Invoke, invoke_source).is_ok());
        assert_eq!(admission.sources.len(), 2);
    }

    #[test]
    fn unit_rate_and_source_limits_remain_invoke_capable() {
        let source: IpAddr = "192.0.2.1".parse().unwrap();
        let forwarding = ForwardAdmission::new(2);

        let mut unit_bursts = limits();
        unit_bursts.global_burst = 1;
        unit_bursts.per_source_burst = 1;
        unit_bursts.source_buckets = 2;
        let admission = Mutex::new(Admission::new(&unit_bursts));

        assert_eq!(
            admit_forwarding(&forwarding, &admission, Route::Challenge, source)
                .err()
                .expect("unit rate bursts make challenge admission unavailable")
                .code,
            "OVERLOADED"
        );
        let before_invoke = admission.lock().unwrap();
        assert_eq!(before_invoke.global.tokens.to_bits(), 1.0_f64.to_bits());
        assert!(before_invoke.sources.is_empty());
        drop(before_invoke);
        assert!(admit_forwarding(&forwarding, &admission, Route::Invoke, source).is_ok());

        let mut unit_source_table = limits();
        unit_source_table.global_burst = 2;
        unit_source_table.per_source_burst = 2;
        unit_source_table.source_buckets = 1;
        let admission = Mutex::new(Admission::new(&unit_source_table));
        assert_eq!(
            admit_forwarding(&forwarding, &admission, Route::Challenge, source)
                .err()
                .expect("a unit source table is reserved for Invoke")
                .code,
            "OVERLOADED"
        );
        let before_invoke = admission.lock().unwrap();
        assert_eq!(before_invoke.global.tokens.to_bits(), 2.0_f64.to_bits());
        assert!(before_invoke.sources.is_empty());
        drop(before_invoke);
        assert!(admit_forwarding(&forwarding, &admission, Route::Invoke, source).is_ok());
    }
    ...
    #[test]
    fn rejected_source_reservation_does_not_consume_the_global_token() {
        let first: IpAddr = "192.0.2.1".parse().unwrap();
        let second: IpAddr = "192.0.2.2".parse().unwrap();
        let mut configured = limits();
        configured.per_source_rate = 1;
        configured.per_source_burst = 1;
        configured.global_rate = 1;
        configured.global_burst = 2;
        let mut admission = Admission::new(&configured);
        ...
        assert!(
            admission.admit(Route::Invoke, second).is_ok(),
            "source A exhaustion must leave the remaining global token for source B"
        );
    }

    #[test]
    fn rejected_global_reservation_does_not_consume_the_source_token() {
        let source: IpAddr = "192.0.2.1".parse().unwrap();
        let mut configured = limits();
        configured.per_source_rate = 1;
        configured.per_source_burst = 2;
        configured.global_rate = 1;
        configured.global_burst = 1;
        let mut admission = Admission::new(&configured);

        assert!(admission.admit(Route::Invoke, source).is_ok());
        assert_eq!(
            admission.admit(Route::Invoke, source).unwrap_err().code,
            "OVERLOADED"
        );
        admission.global.tokens = 1.0;
        assert!(
            admission.admit(Route::Invoke, source).is_ok(),
            "global exhaustion must leave the source reservation available"
        );
    }
