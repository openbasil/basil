...
use basil_cose::{KeyId, SignError, SignatureAlgorithm};
...
use self::qualification::{ArtifactSignQualificationAdapter, QualificationConfig};

const BOOTSTRAP_MAGIC: &[u8; 8] = b"BASILCI\0";
const BOOTSTRAP_VERSION: u8 = 1;
const BOOTSTRAP_COMMIT: u8 = 1;
const BOOTSTRAP_FIELDS: usize = 4;
const MAX_BOOTSTRAP_BYTES: usize = 64 * 1024;
const MAX_BOOTSTRAP_FIELD_BYTES: usize = 32 * 1024;
...
impl ProviderKind {
    fn parse(value: &str) -> Result<Self> {
        match value {
            ...
            "forgejoActions" => Ok(Self::ForgejoActions),
            _ => bail!("CI session bootstrap provider kind is unsupported"),
        }
    }
...
struct BootstrapFrame {
    ...
    expected_origin: Zeroizing<String>,
...
pub struct SessionArgs {
    /// Fixed `/proc/self/exe` path used for executable self-verification.
    #[arg(long)]
    ...
    /// Optional root-owned fixed-purpose qualification configuration.
    #[arg(long)]
    pub qualification_config: Option<PathBuf>,
}

/// Immutable configuration for one CI session.
#[derive(Clone, Debug)]
struct SessionConfig {
    /// Fixed executable self-reference, or an injected path in unit tests.
    pub basil_executable: PathBuf,
...
impl TryFrom<SessionArgs> for SessionConfig {
    ...
    fn try_from(args: SessionArgs) -> Result<Self> {
        ...
        if args.basil_executable != Path::new("/proc/self/exe") {
            bail!("CI session executable provenance must use /proc/self/exe");
        }
        Ok(Self {
...
impl SessionIdentity {
    fn provider_token(&self) -> &str {
        &self.token
    }

    fn proof(&self) -> &ProofIdentity {
        &self.proof
    }
...
trait TypedInvocationTransport: Send + Sync + 'static {
    ...
    /// Maximum accepted request-frame body length.
    const REQUEST_MAX_BYTES: usize;
    /// Maximum emitted response-frame body length.
    const RESPONSE_MAX_BYTES: usize;
    ...
    /// Irrevocably reserve an adapter attempt before decoding its body.
    fn claim_request(&self) -> Result<(), ()> {
        Ok(())
    }

    /// Perform one invocation with the current identity.
    fn invoke(
        &self,
...
/// OIDC token source backed by the provider bootstrap frame.
...
impl ActionsOidcTokenSource {
    fn from_bootstrap(frame: BootstrapFrame) -> Result<Self> {
        if frame.request_token.is_empty() || frame.request_token.len() > MAX_TOKEN_BYTES {
            bail!("provider token-request bearer has an invalid length");
        }
        let expected_origin = parse_expected_origin(&frame.expected_origin)?;
        validate_token_request_url(&frame.request_url, &expected_origin)?;
        ...
        Ok(Self {
            _provider_kind: frame.provider_kind,
            expected_origin,
            request_url: frame.request_url,
            request_token: frame.request_token,
            client,
        })
    }
    ...
    async fn fetch_from_url(
        ...
        validate_request_origin(base, &self.expected_origin)?;
...
impl IdentityTokenSource for ActionsOidcTokenSource {
    async fn fetch(&self, proof_audience: &str) -> Result<Zeroizing<String>> {
        let base = validate_token_request_url(&self.request_url, &self.expected_origin)?;
...
impl BootstrapInput {
    fn stdin() -> Result<Self> {
        let descriptor =
            rustix::io::dup(std::io::stdin()).context("duplicating bootstrap stdin")?;
        let flags = rustix::fs::fcntl_getfl(&descriptor)?;
        rustix::fs::fcntl_setfl(&descriptor, flags | OFlags::NONBLOCK)?;
        Ok(Self {
            descriptor: AsyncFd::new(descriptor).context("registering bootstrap stdin")?,
        })
    }
    ...
    async fn read(&self, buffer: &mut [u8]) -> Result<usize> {
        loop {
            let mut ready = self
                ...
                .readable()
                .await
                .context("waiting for CI session bootstrap")?;
            match ready.try_io(|descriptor| {
                rustix::io::read(descriptor.get_ref(), &mut *buffer).map_err(std::io::Error::from)
            }) {
                Ok(result) => return result.context("reading CI session bootstrap"),
                Err(_would_block) => {}
            }
        }
    }

    async fn read_exact(&self, buffer: &mut [u8]) -> Result<()> {
        let mut offset = 0;
        while offset < buffer.len() {
            let read = self
                .read(
                    buffer
                        .get_mut(offset..)
                        .ok_or_else(|| anyhow!("invalid CI session bootstrap offset"))?,
                )
                .await?;
            if read == 0 {
                bail!("CI session bootstrap is truncated");
            }
            offset = offset
                .checked_add(read)
                .ok_or_else(|| anyhow!("CI session bootstrap length overflows"))?;
        }
        Ok(())
    }
}

async fn read_bootstrap_frame(input: &BootstrapInput) -> Result<BootstrapFrame> {
    const HEADER_BYTES: usize = BOOTSTRAP_MAGIC.len() + 1 + BOOTSTRAP_FIELDS * 4;
    let mut header = Zeroizing::new([0_u8; HEADER_BYTES]);
    input.read_exact(&mut *header).await?;
    if header.get(..BOOTSTRAP_MAGIC.len()) != Some(BOOTSTRAP_MAGIC.as_slice())
        || header.get(BOOTSTRAP_MAGIC.len()).copied() != Some(BOOTSTRAP_VERSION)
    {
        bail!("CI session bootstrap version is unsupported");
    }
    let mut lengths = [0_usize; BOOTSTRAP_FIELDS];
    let mut total = HEADER_BYTES;
    for (index, length) in lengths.iter_mut().enumerate() {
        let start = BOOTSTRAP_MAGIC.len() + 1 + index * 4;
        let bytes: [u8; 4] = header
            .get(start..start + 4)
            .ok_or_else(|| anyhow!("CI session bootstrap length is truncated"))?
            .try_into()
            .map_err(|_| anyhow!("CI session bootstrap length is invalid"))?;
        *length = usize::try_from(u32::from_be_bytes(bytes))?;
        if *length == 0 || *length > MAX_BOOTSTRAP_FIELD_BYTES {
            bail!("CI session bootstrap field length is invalid");
        }
        total = total
            .checked_add(*length)
            .ok_or_else(|| anyhow!("CI session bootstrap length overflows"))?;
    }
    if total > MAX_BOOTSTRAP_BYTES || lengths[3] > MAX_TOKEN_BYTES {
        bail!("CI session bootstrap exceeds its bound");
    }
    let mut fields = Vec::with_capacity(BOOTSTRAP_FIELDS);
    for length in lengths {
        let mut bytes = Zeroizing::new(vec![0_u8; length]);
        input.read_exact(&mut bytes).await?;
        let text = std::str::from_utf8(&bytes)
            .map_err(|_| anyhow!("CI session bootstrap field is not UTF-8"))?;
        fields.push(Zeroizing::new(text.to_owned()));
    }
    let provider_kind = ProviderKind::parse(
        fields
            .first()
            .ok_or_else(|| anyhow!("CI session bootstrap provider is absent"))?,
    )?;
    let mut fields = fields.into_iter();
    let _provider = fields.next();
    let expected_origin = fields
        .next()
        .ok_or_else(|| anyhow!("CI session bootstrap origin is absent"))?;
    let request_url = fields
        .next()
        .ok_or_else(|| anyhow!("CI session bootstrap URL is absent"))?;
    let request_token = fields
        .next()
        .ok_or_else(|| anyhow!("CI session bootstrap bearer is absent"))?;
    if fields.next().is_some() {
        bail!("CI session bootstrap has trailing fields");
    }
    Ok(BootstrapFrame {
        provider_kind,
        expected_origin,
...
async fn read_bootstrap_commit(input: &BootstrapInput) -> Result<()> {
    let mut acknowledgement = Zeroizing::new([0_u8; 1]);
    input.read_exact(&mut *acknowledgement).await?;
    if acknowledgement.first().copied() != Some(BOOTSTRAP_COMMIT) {
        bail!("CI session bootstrap commit acknowledgement is invalid");
    }
    let mut trailing = Zeroizing::new([0_u8; 1]);
    if input.read(&mut *trailing).await? != 0 {
        bail!("CI session bootstrap has trailing bytes");
    }
    Ok(())
}
    ...
    S: IdentityTokenSource,
{
    ...
    async fn serve(self, mut signals: SessionSignals) -> Result<()> {
        ...
        let mut next_refresh = initial_refresh(&self.identity).await?;
        let mut refresh = None;
        let result = 'serving: loop {
            if let Err(error) = reclaim_ready_session_tasks(&mut tasks) {
                break 'serving Err(error);
            }
            let expiry = self.identity.read().await.valid_until_unix();
            let refresh_sleep =
                tokio::time::sleep_until(unix_to_instant(next_refresh.unwrap_or(expiry)));
            ...
            tokio::select! {
                ...
                refreshed = async {
                    ...
                    refresh = None;
                    let schedule = schedule_after_refresh(refreshed, &self.identity).await;
                    next_refresh = match schedule {
                        Ok(next) => next,
                        Err(error) => break Err(error),
                    };
                }
                () = refresh_sleep, if next_refresh.is_some() && refresh.is_none() => {
                    next_refresh = None;
                    refresh = Some(Box::pin(refresh_identity(
                ...
                accepted = accept_adapter(self.adapter_listener.as_ref()), if tasks.len() < MAX_SESSION_CONNECTIONS => {
                    match accepted {
                        Ok(stream) => {
                            let deadline = tokio::time::Instant::now() + SESSION_OPERATION_DEADLINE;
                            if spawn_adapter_task(
                                &mut tasks,
                                stream,
                                Arc::clone(&self.transport),
                                Arc::clone(&self.identity),
                                Arc::clone(&self.connection_slots),
                                Arc::clone(&self.adapter_slot),
                                deadline,
                            ) {
                                #[cfg(test)]
                                self.task_set_high_water.observe(tasks.len());
                ...
                completed = tasks.join_next(), if !tasks.is_empty() => {
                    let Some(completed) = completed else {
                        break Err(anyhow!("CI session task set closed unexpectedly"));
                    };
        ...
        drop(refresh);
        finish_session_tasks(shutdown_tx, &mut tasks).await;
        drop(tasks);
        result
    }
}
    ...
    S: IdentityTokenSource,
{
    ...
    verify_executable(
        ...
        if config.basil_executable == Path::new("/proc/self/exe") {
            0
        } else {
...
/// Run the production session with no adapter by default.
///
/// A validated qualification configuration enables the sole compile-time
/// artifact-sign qualification adapter before provider-token acquisition.
pub async fn run(args: SessionArgs) -> Result<()> {
    let qualification_path = args.qualification_config.clone();
    let config: SessionConfig = args.try_into()?;
    let qualification = qualification_path
        .as_deref()
        .map(QualificationConfig::load)
        .transpose()?;
    let signals = SessionSignals::register()?;
    let input = BootstrapInput::stdin()?;
    let bootstrap = read_bootstrap_frame(&input).await?;
    if let Some(qualification) = &qualification {
        qualification.cross_check(
            bootstrap.provider_kind.config_name(),
            &bootstrap.expected_origin,
            config.rule_max_token_age.as_secs(),
        )?;
    }
    install_rustls_provider();
    let adapter = qualification
        .map(ArtifactSignQualificationAdapter::new)
        .transpose()?;
    let token_source = ActionsOidcTokenSource::from_bootstrap(bootstrap)?;
    match adapter {
        Some(adapter) => run_session(signals, &input, config, token_source, adapter).await,
        None => run_session(signals, &input, config, token_source, NoAdapter).await,
    }
}

async fn run_session<T, S>(
    mut signals: SessionSignals,
...
where
    ...
    S: IdentityTokenSource,
{
    let mut preparation = Box::pin(prepare_session(config, token_source, transport));
    let mut commit = Box::pin(read_bootstrap_commit(input));
    let session = tokio::select! {
        ...
        result = &mut commit => {
            result?;
            bail!("CI session bootstrap was acknowledged before outputs");
        },
        result = &mut preparation => result?,
    };
    {
        let mut stdout = std::io::stdout().lock();
        serde_json::to_writer(&mut stdout, session.outputs())?;
        writeln!(&mut stdout)?;
        stdout.flush()?;
    }
    tokio::select! {
        biased;
        result = signals.wait() => return result,
...
impl TypedInvocationTransport for NoAdapter {
    ...
    const REQUEST_MAX_BYTES: usize = 1;
    const RESPONSE_MAX_BYTES: usize = 1;
...
impl ProofIdentity {
    fn generate() -> Result<Self> {
        ...
        let jkt_bytes: [u8; 32] = Sha256::digest(canonical.as_bytes()).into();
        let jkt = base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(jkt_bytes);
        let audience = format!("urn:basil:ci:jkt:{jkt}");
        let key_id = KeyId::from_text(&jkt).context("building CI proof key ID")?;
        Ok(Self {
            signing_key,
            key_id,
            jkt_bytes,
...
impl basil_cose::Signer for ProofIdentity {
    ...
    async fn sign(
        &self,
        sig_structure: &[u8],
    ) -> std::result::Result<basil_cose::Signature, SignError> {
        let signature = self.signing_key.sign(sig_structure);
        basil_cose::Signature::from_bytes(signature.to_bytes().to_vec()).map_err(|_| {
            SignError::Provider {
                message: "CI proof signing failed".to_string(),
            }
        })
    }
}
...
async fn schedule_after_refresh(
    refreshed: Result<u64>,
    identity: &RwLock<Arc<SessionIdentity>>,
) -> Result<Option<u64>> {
    if let Ok(valid_until) = refreshed {
        return Ok(refresh_after_success_at(valid_until, unix_now()?));
    }
    let current_expiry = identity.read().await.valid_until_unix();
    let now = unix_now()?;
    if now >= current_expiry {
        bail!("provider identity expired during refresh");
    }
    Ok(retry_refresh_at(current_expiry, now))
}

async fn initial_refresh(identity: &RwLock<Arc<SessionIdentity>>) -> Result<Option<u64>> {
    Ok(refresh_at(
        identity.read().await.valid_until_unix(),
        unix_now()?,
    ))
}
...
fn parse_expected_origin(value: &str) -> Result<ExpectedOrigin> {
    ...
    {
        bail!("expected provider token-request origin is not an exact HTTPS origin");
    }
    origin_of(&url)
}

fn origin_of(url: &reqwest::Url) -> Result<ExpectedOrigin> {
    Ok(ExpectedOrigin {
        scheme: url.scheme().to_owned(),
        host: url
            .host_str()
            .ok_or_else(|| anyhow!("provider token-request URL has no host"))?
            .to_owned(),
        effective_port: url
            .port_or_known_default()
            .ok_or_else(|| anyhow!("provider token-request URL has no effective port"))?,
    })
}

fn validate_request_origin(url: &reqwest::Url, expected: &ExpectedOrigin) -> Result<()> {
    if &origin_of(url)? != expected {
        bail!("provider token-request URL origin does not match the expected origin");
    }
    Ok(())
}

fn validate_token_request_url(
    value: &str,
    expected_origin: &ExpectedOrigin,
) -> Result<reqwest::Url> {
    let url = reqwest::Url::parse(value).context("parsing provider token-request URL")?;
    ...
    validate_request_origin(&url, expected_origin)?;
    Ok(url)
}
...
fn spawn_adapter_task<T: TypedInvocationTransport>(
    tasks: &mut JoinSet<OwnedSemaphorePermit>,
    stream: UnixStream,
    transport: Arc<T>,
    identity: Arc<RwLock<Arc<SessionIdentity>>>,
    connection_slots: Arc<Semaphore>,
    ...
    let Ok(adapter_permit) = adapter_slot.try_acquire_owned() else {
        return false;
    };
    ...
    tasks.spawn(async move {
        let permit = handle_adapter_bounded(stream, transport, identity, permit, deadline).await;
        drop(adapter_permit);
        permit
    });
    true
...
fn reclaim_ready_session_tasks(tasks: &mut JoinSet<OwnedSemaphorePermit>) -> Result<()> {
    while let Some(completed) = tasks.try_join_next() {
        reclaim_session_task(completed)?;
    }
    Ok(())
}
...
async fn handle_adapter_inner<T: TypedInvocationTransport>(
    ...
    let Ok(request) = read_adapter_request(stream, transport.as_ref()).await else {
        let _ = write_frame(
            stream,
            &AdapterResponse::<()>::Rejected,
            T::RESPONSE_MAX_BYTES,
        )
        .await;
        return;
    };
    ...
    let response = match enforce_invocation_deadline(
        connection_deadline,
        valid_until,
        transport.invoke(current, request),
    )
    .await
    {
    ...
    let _ = write_frame(stream, &response, T::RESPONSE_MAX_BYTES).await;
}

async fn read_adapter_request<T: TypedInvocationTransport>(
    stream: &mut UnixStream,
    transport: &T,
) -> Result<T::Request, ()> {
    transport.claim_request()?;
    let body = read_frame_bytes(stream, T::REQUEST_MAX_BYTES).await?;
    T::decode_request(&body)
}
...
fn refresh_at(valid_until: u64, now: u64) -> Option<u64> {
    if now >= valid_until {
        return None;
    }
...
fn refresh_after_success_at(valid_until: u64, now: u64) -> Option<u64> {
    refresh_at(valid_until, now).filter(|scheduled| *scheduled > now)
}

const fn retry_refresh_at(valid_until: u64, now: u64) -> Option<u64> {
    let retry = now.saturating_add(1);
    if retry < valid_until {
        Some(retry)
    } else {
        None
    }
...
fn verify_executable(source: &Path, expected: &str, required_uid: u32) -> Result<()> {
    ...
    let source_fd = rustix::fs::open(source, flags, Mode::empty())
    ...
    if FileType::from_raw_mode(source_stat.st_mode) != FileType::RegularFile
        || source_stat.st_uid != required_uid
        || source_stat.st_mode & 0o111 == 0
        || source_stat.st_mode & 0o022 != 0
    {
        bail!("reviewed Basil path is not a root-owned non-writable regular executable");
    }
...
mod tests {
    ...
    fn bootstrap_bytes(fields: &[&str; BOOTSTRAP_FIELDS]) -> Zeroizing<Vec<u8>> {
        let mut bytes = Zeroizing::new(Vec::new());
        bytes.extend_from_slice(BOOTSTRAP_MAGIC);
        bytes.push(BOOTSTRAP_VERSION);
        for field in fields {
    ...
    fn bootstrap_input_pair() -> (BootstrapInput, UnixStream) {
        let (server, client) = UnixStream::pair().unwrap();
        let descriptor = rustix::io::dup(&server).unwrap();
        drop(server);
        (
            BootstrapInput {
                descriptor: AsyncFd::new(descriptor).unwrap(),
            },
            client,
        )
    }
    ...
    #[tokio::test]
    async fn bootstrap_frame_is_closed_bounded_and_commit_requires_eof() {
        let fields = [
            "github",
        ...
        let (input, mut writer) = bootstrap_input_pair();
        writer.write_all(&bootstrap_bytes(&fields)).await.unwrap();
        let frame = read_bootstrap_frame(&input).await.unwrap();
        assert_eq!(frame.provider_kind, ProviderKind::Github);
        assert_eq!(&*frame.expected_origin, fields[1]);
        assert_eq!(&*frame.request_url, fields[2]);
        assert_eq!(&*frame.request_token, fields[3]);
        writer.write_all(&[BOOTSTRAP_COMMIT]).await.unwrap();
        writer.shutdown().await.unwrap();
        read_bootstrap_commit(&input).await.unwrap();

        for mutation in ["unknown", "truncated", "oversized"] {
            let (input, mut writer) = bootstrap_input_pair();
            let mut bytes = bootstrap_bytes(&fields);
            if mutation == "unknown" {
                bytes[BOOTSTRAP_MAGIC.len()] = BOOTSTRAP_VERSION + 1;
            } else if mutation == "truncated" {
                let truncated = bytes.len() - 1;
                bytes.truncate(truncated);
            } else {
                assert_eq!(mutation, "oversized");
                let start = BOOTSTRAP_MAGIC.len() + 1 + 3 * 4;
                bytes[start..start + 4]
                    .copy_from_slice(&u32::try_from(MAX_TOKEN_BYTES + 1).unwrap().to_be_bytes());
            }
            writer.write_all(&bytes).await.unwrap();
            writer.shutdown().await.unwrap();
            assert!(read_bootstrap_frame(&input).await.is_err(), "{mutation}");
        }

        let (input, mut writer) = bootstrap_input_pair();
        writer.write_all(&[BOOTSTRAP_COMMIT, 2]).await.unwrap();
        writer.shutdown().await.unwrap();
        assert!(read_bootstrap_commit(&input).await.is_err());
        let (input, mut writer) = bootstrap_input_pair();
        writer.shutdown().await.unwrap();
        assert!(read_bootstrap_commit(&input).await.is_err());
    }
    ...
    fn test_oidc_source(url: &reqwest::Url, secret: &str) -> ActionsOidcTokenSource {
        install_rustls_provider();
        ActionsOidcTokenSource {
            _provider_kind: ProviderKind::Github,
            expected_origin: origin_of(url).unwrap(),
            request_url: Zeroizing::new(url.to_string()),
            request_token: Zeroizing::new(secret.to_owned()),
    ...
    fn audience_query_is_exact_and_encoded_once() {
        let expected = parse_expected_origin("https://issuer.example").unwrap();
        let base =
            validate_token_request_url("https://issuer.example/token?job=1", &expected).unwrap();
        ...
        for rejected in [
            ...
            assert!(
                validate_token_request_url(rejected, &expected).is_err(),
                "{rejected}"
            );
        }
    }
    ...
    #[tokio::test]
    async fn origin_mismatch_fails_before_any_http_request() {
        let (url, request, server) =
            raw_http_server(http_response("200 OK", br#"{"value":"x"}"#)).await;
        let expected_url = reqwest::Url::parse("https://issuer.example/token").unwrap();
        let source = test_oidc_source(&expected_url, "request-bearer-secret");
        let error = source
    ...
    async fn provider_http_is_bounded_strict_non_redirecting_and_secret_safe() {
        ...
        let source = test_oidc_source(&url, bearer);
        let fetched = source
            .fetch_from_url(&url, "urn:basil:ci:jkt:test")
            .await
            .unwrap();
    ...
    #[test]
    fn proof_is_stable_across_provider_token_refresh() {
    ...
    #[test]
    fn refresh_and_retry_scheduling_is_strictly_before_expiry() {
        assert_eq!(refresh_at(100, 100), None);
        assert_eq!(refresh_at(101, 100), Some(100));
        assert_eq!(refresh_at(200, 100), Some(180));
        assert!(refresh_at(200, 100).is_some_and(|scheduled| scheduled < 200));
        assert_eq!(refresh_after_success_at(101, 100), None);
        assert_eq!(refresh_after_success_at(102, 100), Some(101));
        assert_eq!(retry_refresh_at(101, 100), None);
        assert_eq!(retry_refresh_at(102, 100), Some(101));
    }

    struct InitialThenStalledSource {
        initial: Mutex<Option<Zeroizing<String>>>,
        refresh_started: Arc<AtomicBool>,
    }

    impl IdentityTokenSource for InitialThenStalledSource {
        async fn fetch(&self, _proof_audience: &str) -> Result<Zeroizing<String>> {
            let token = self.initial.lock().unwrap().take();
    ...
    #[tokio::test]
    async fn in_flight_refresh_cannot_mask_identity_expiry() {
        let root = test_directory("stalled-refresh");
        let source = root.join("source");
        std::fs::write(&source, b"binary").unwrap();
        std::fs::set_permissions(&source, std::fs::Permissions::from_mode(0o500)).unwrap();
        let now = unix_now().unwrap();
        ...
        let session = prepare_session(
            SessionConfig {
                basil_executable: source.clone(),
                basil_executable_sha256: hex::encode(Sha256::digest(b"binary")),
                rule_max_token_age: Duration::from_secs(2),
                runtime_parent: root.clone(),
            },
            InitialThenStalledSource {
                initial: Mutex::new(Some(jwt(now, now + 2, "stalled-refresh"))),
    ...
    #[test]
    fn executable_is_verified_in_place_without_a_runtime_copy() {
        let root = test_directory("in-place");
        let source = root.join("source");
        ...
        verify_executable(&source, &digest, rustix::process::geteuid().as_raw()).unwrap();
        assert!(!runtime.path().join("basil").exists());
    ...
    fn executable_provenance_rejects_mutable_and_wrong_bytes() {
        ...
        assert!(verify_executable(&source, &digest, uid).is_err());
        std::fs::set_permissions(&source, std::fs::Permissions::from_mode(0o500)).unwrap();
        assert!(verify_executable(&source, &"0".repeat(64), uid).is_err());
        assert!(verify_executable(&source, &digest, uid.wrapping_add(1)).is_err());
        std::fs::remove_file(source).unwrap();
        std::fs::remove_dir(root).unwrap();
    }
