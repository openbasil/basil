...
use basil_core::core::nix_cache_file::{
    LockedCacheRoot, NarinfoCommit, NarinfoEdit, NarinfoMutation, ReadOnlyCacheRoot, edit_narinfo,
};
...
pub async fn run(socket: &str, command: NixCacheCommand) -> Result<()> {
    ...
    let outcome = match command {
        NixCacheCommand::Remove(args) => run_remove(&args, &mut output),
        NixCacheCommand::Sign(args) => {
            run_connected(socket, SigningCommand::Sign(args), &mut output).await
        }
        NixCacheCommand::Replace(args) => {
            run_connected(socket, SigningCommand::Replace(args), &mut output).await
        }
    };
    let output_result = std::io::stdout()
        .lock()
        .write_all(&output)
        .context("writing Nix cache mutation output");
    outcome?;
    output_result
}
...
async fn run_connected<W: Write>(
    ...
    run_signing(&mut client, command, &mut SystemIds, output).await
}
...
async fn run_signing<C: CacheRpc, I: IdSource, W: Write>(
    ...
    let describe_request_id = ids.fresh(&used_ids)?;
    used_ids.insert(describe_request_id);
    let identity = client
        .describe_nix_cache_key(key_id, batch_id, describe_request_id)
        .await
    ...
    if selection.dry_run {
        ...
        return preview_batch(&root, &selected, Some(&identity), operation, output);
    }
    ...
    for target in selected {
        let commit =
            mutate_signing_target(&root, &target, old_names.as_deref(), &mut batch).await?;
        render_commit(&target.relative, commit, output)?;
    }
    Ok(())
}

struct SigningBatch<'a, C, I> {
    client: &'a mut C,
    ids: &'a mut I,
...
async fn mutate_signing_target<C: CacheRpc, I: IdSource>(
    root: &LockedCacheRoot,
    target: &SelectedNarinfo,
    old_names: Option<&[&str]>,
    batch: &mut SigningBatch<'_, C, I>,
    ...
    let signature = match (old_names, existing) {
        (None, ExistingSignature::Verified(_)) => return Ok(NarinfoCommit::Unchanged),
        ...
        (_, ExistingSignature::Verified(value)) => value,
    ...
    commit_edit(root, &target.relative, &snapshot, edit)
}
...
async fn sign_and_verify<C: CacheRpc, I: IdSource>(
    client: &mut C,
    ids: &mut I,
    ...
    let request_id = ids.fresh(used_ids)?;
    used_ids.insert(request_id);
    let response = client
        .sign_nix_cache_fingerprint(key_id, fingerprint.as_bytes(), batch_id, request_id)
        .await
        .with_context(|| format!("signing Nix path-info fingerprint with {key_id}"))?;
    if response.key != *identity {
        bail!("Basil returned a different Nix cache key identity while signing");
    }
    verify_raw_signature(identity, fingerprint.as_bytes(), &response.signature)
    ...
    for target in selected {
        ...
        let edit = match operation {
            Operation::Remove { key_names } => {
                edit_narinfo(bytes, NarinfoMutation::Remove { key_names })?
            }
            Operation::Sign => {
                ...
                match inspect_existing_signature(bytes, identity, fingerprint.as_bytes()) {
                    ExistingSignature::Verified(_) => NarinfoEdit::Unchanged,
                    ExistingSignature::Conflict => return signature_conflict(&target.relative),
                    ExistingSignature::Absent => NarinfoEdit::Changed(Vec::new()),
                }
            }
            Operation::Replace { old_key_names } => {
                ...
                let new_name_is_removed =
                    old_key_names.iter().any(|name| *name == identity.key_name);
                match inspect_existing_signature(bytes, identity, fingerprint.as_bytes()) {
                    ExistingSignature::Verified(signature) => edit_narinfo(
                        bytes,
...
fn render_commit<W: Write>(relative: &Path, commit: NarinfoCommit, output: &mut W) -> Result<()> {
    match commit {
        NarinfoCommit::Unchanged => writeln!(output, "{}: unchanged", relative.display())?,
        NarinfoCommit::Written => writeln!(output, "{}: written", relative.display())?,
