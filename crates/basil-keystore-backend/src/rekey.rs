...
//! (`basil keystore rekey`) composes these primitives into the bound v2 bundle
//! transition, including authenticated pre- and post-transition authority.
//!
//! # Caller obligations
//!
//! - **Quiescence**: stop or quiesce the agent operationally, then acquire the
//!   exclusive [`RekeyLock`]. A socket probe is only a user-experience
//!   preflight and cannot replace the lock; the shared/exclusive lock is the
//!   synchronization boundary that excludes store-lifetime openers.
...
//! - **Directory custody**: keep the database directory owner-only and require
//!   every same-EUID participant to honor [`RekeyLock`]. Descriptor/inode
//!   revalidation detects substitutions visible before cleanup, but Linux has
//!   no atomic unlink-if-inode operation. A same-EUID process that ignores the
//!   advisory lock can race the final check and can already rewrite or remove
//!   the database, marker, lock, and sidecars directly.
...
/// Maximum UTF-8 byte length of a catalog backend identifier bound into a
/// rekey transition.
const BACKEND_ID_MAX_BYTES: usize = 256;
...
pub enum KeystoreRekeyError {
    ...
    /// The authenticated sealed-bundle transition input is incomplete or
    /// inconsistent with the staged database copy.
    #[error("rekey bundle transition binding invalid: {detail}")]
    BundleBindingInvalid {
        /// Which structural check failed; identifier and digest values are
        /// never included.
        detail: String,
    },
    /// The complete marker was atomically published, but a subsequent
    /// identity or durability confirmation failed. Recovery must treat the
    /// named marker as the fence and inspect it rather than retry creation.
    #[error(
        "rekey intent marker published but durability confirmation failed: {detail}; run \
         `basil keystore rekey --resume`"
    )]
    MarkerPublished {
        /// Bounded operation detail; marker contents are never included.
        detail: String,
    },
...
impl SensitiveDek {
    ...
    /// Copy this DEK into the sealed-bundle secret container.
    ///
    /// The returned owner wipes its storage on drop; no raw byte view is
    /// exposed to the caller.
    #[must_use]
    pub fn to_secret_array(&self) -> SecretArray<32> {
        SecretArray::new(*self.bytes)
    }
...
fn validate_lock_target(
    ...
    validate_lock_entry(&lock.fd, db_dir, db_name)
}
...
fn validate_lock_entry(
    ...
    let entry = rustix::fs::statat(db_dir, name.as_str(), AtFlags::SYMLINK_NOFOLLOW)
        .map_err(|errno| fs_err("stat rekey lock file", errno))?;
    let held = rustix::fs::fstat(fd).map_err(|errno| fs_err("fstat rekey lock file", errno))?;
    let sane = FileType::from_raw_mode(held.st_mode) == FileType::RegularFile
        && held.st_uid == rustix::process::geteuid().as_raw()
        && held.st_nlink == 1
        && held.st_mode.trailing_zeros() >= 6;
    if entry.st_dev != held.st_dev || entry.st_ino != held.st_ino || !sane {
        return Err(KeystoreRekeyError::TargetMismatch {
            detail: "the rekey lock pathname or held-inode metadata changed after acquisition"
                .to_owned(),
        });
    }
    Ok(())
...
impl RekeyLock {
    ...
    pub fn acquire_exclusive(
        ...
        validate_lock_entry(&fd, db_dir, db_name)?;
        Ok(Self {
            fd,
...
const DATABASE_DIRECTORY_FLAGS: OFlags = OFlags::RDONLY
    .union(OFlags::DIRECTORY)
    .union(OFlags::NOFOLLOW)
    .union(OFlags::CLOEXEC);

fn unsafe_database_directory(detail: &'static str) -> KeystoreRekeyError {
    KeystoreRekeyError::Backend {
        detail: detail.to_owned(),
    }
}

fn validate_database_directory(
    directory: BorrowedFd<'_>,
    is_database_directory: bool,
    freshly_created: bool,
) -> Result<(), KeystoreRekeyError> {
    let stat = rustix::fs::fstat(directory)
        .map_err(|errno| fs_err("fstat database directory component", errno))?;
    let effective_uid = rustix::process::geteuid().as_raw();
    let mode = stat.st_mode & 0o7777;
    if FileType::from_raw_mode(stat.st_mode) != FileType::Directory {
        return Err(unsafe_database_directory(
            "database path component is not a directory",
        ));
    }
    if stat.st_uid != 0 && stat.st_uid != effective_uid {
        return Err(unsafe_database_directory(
            "database path component is not owned by root or the effective user",
        ));
    }
    let root_sticky_ancestor = !is_database_directory && stat.st_uid == 0 && mode & 0o1000 != 0;
    if mode & 0o022 != 0 && !root_sticky_ancestor {
        return Err(unsafe_database_directory(
            "database path component is group- or world-writable",
        ));
    }
    if is_database_directory && mode & 0o077 != 0 {
        return Err(unsafe_database_directory(
            "database directory must be accessible only by its owner",
        ));
    }
    if stat.st_nlink == 0 {
        return Err(unsafe_database_directory(
            "database path component was unlinked while being opened",
        ));
    }
    if freshly_created && (stat.st_uid != effective_uid || mode & 0o777 != 0o700) {
        return Err(unsafe_database_directory(
            "new database directory failed ownership or 0700 mode checks",
        ));
    }
    Ok(())
}

trait DatabaseDirectorySync {
    fn sync(
        &self,
        directory: BorrowedFd<'_>,
...
struct FilesystemDatabaseDirectorySync;

impl DatabaseDirectorySync for FilesystemDatabaseDirectorySync {
    fn sync(
        &self,
        directory: BorrowedFd<'_>,
        operation: &'static str,
    ) -> Result<(), KeystoreRekeyError> {
        rustix::fs::fsync(directory).map_err(|errno| fs_err(operation, errno))
    }
}

fn sync_accepted_database_directory(
    sync: &impl DatabaseDirectorySync,
    parent: BorrowedFd<'_>,
    directory: BorrowedFd<'_>,
) -> Result<(), KeystoreRekeyError> {
    sync.sync(directory, "sync database directory component")?;
    sync.sync(parent, "sync database directory parent")
}

fn open_database_directory_component(
    parent: BorrowedFd<'_>,
    name: &std::ffi::OsStr,
    is_database_directory: bool,
    sync: &impl DatabaseDirectorySync,
) -> Result<OwnedFd, KeystoreRekeyError> {
    match rustix::fs::openat(parent, name, DATABASE_DIRECTORY_FLAGS, Mode::empty()) {
        Ok(directory) => {
            validate_database_directory(directory.as_fd(), is_database_directory, false)?;
            sync_accepted_database_directory(sync, parent, directory.as_fd())?;
            Ok(directory)
        }
        Err(Errno::NOENT) => {
            create_database_directory_component(parent, name, is_database_directory, sync)
        }
        Err(errno) => Err(fs_err("open database directory component", errno)),
    }
}

fn create_database_directory_component(
    parent: BorrowedFd<'_>,
    name: &std::ffi::OsStr,
    is_database_directory: bool,
    sync: &impl DatabaseDirectorySync,
    ...
    match rustix::fs::mkdirat(parent, name, Mode::from_raw_mode(0o700)) {
        ...
        Err(Errno::EXIST) => {
            let directory =
                rustix::fs::openat(parent, name, DATABASE_DIRECTORY_FLAGS, Mode::empty())
                    .map_err(|errno| fs_err("open raced database directory component", errno))?;
            validate_database_directory(directory.as_fd(), is_database_directory, false)?;
            sync_accepted_database_directory(sync, parent, directory.as_fd())?;
            return Ok(directory);
        }
        Err(errno) => return Err(fs_err("create database directory component", errno)),
    }

    let directory = match rustix::fs::openat(parent, name, DATABASE_DIRECTORY_FLAGS, Mode::empty())
    {
        Ok(directory) => directory,
        Err(errno) => {
            let _ = rustix::fs::unlinkat(parent, name, AtFlags::REMOVEDIR);
            return Err(fs_err("open new database directory component", errno));
        }
    };
    if let Err(errno) = rustix::fs::fchmod(&directory, Mode::from_raw_mode(0o700)) {
        drop(directory);
        let _ = rustix::fs::unlinkat(parent, name, AtFlags::REMOVEDIR);
        return Err(fs_err("set new database directory mode", errno));
    }
    if let Err(error) = validate_database_directory(directory.as_fd(), is_database_directory, true)
    {
        drop(directory);
        let _ = rustix::fs::unlinkat(parent, name, AtFlags::REMOVEDIR);
        return Err(error);
...
fn open_or_create_database_directory_with(
    db_path: &Path,
    sync: &impl DatabaseDirectorySync,
) -> Result<OwnedFd, KeystoreRekeyError> {
    if !db_path.is_absolute() {
        return Err(unsafe_database_directory(
            "database path must be absolute on Linux",
        ));
    }
    let parent = db_path
        .parent()
    ...
    {
        return Err(unsafe_database_directory(
            "database directory path contains a non-plain component",
        ));
    }
    let component_count = parent
        ...
        .count();
    let mut directory = rustix::fs::open(Path::new("/"), DATABASE_DIRECTORY_FLAGS, Mode::empty())
        .map_err(|errno| fs_err("open database path root", errno))?;
    validate_database_directory(directory.as_fd(), component_count == 0, false)?;

    let mut opened_components = 0_usize;
    for component in parent.components() {
        match component {
            Component::RootDir | Component::CurDir => {}
            Component::Normal(name) => {
                opened_components = opened_components.saturating_add(1);
                directory = open_database_directory_component(
                    directory.as_fd(),
                    name,
                    opened_components == component_count,
                    sync,
                )?;
            }
...
fn guard_store_open_with(
    ...
    // The component walk pins each ancestor with O_NOFOLLOW. Missing
    // components are created relative to the last trusted descriptor, then
    // fixed at 0700 and synced before the walk continues.
    let db_dir = open_or_create_database_directory_with(db_path, sync)?;
...
pub(crate) fn guard_store_open(db_path: &Path) -> Result<OwnedFd, KeystoreRekeyError> {
    guard_store_open_with(db_path, &FilesystemDatabaseDirectorySync)
}
...
/// Authenticated sealed-bundle transition bound to one staged database rekey.
///
/// The caller obtains these values from the opened pre-reseal bundle and the
/// durably written post-reseal bundle. The catalog backend identifier is kept
/// byte-exact; marker serialization encodes it as bounded lowercase hex.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BundleTransitionBinding {
    bundle_id: [u8; 16],
    backend_id: String,
    pre_bundle_b3: [u8; 32],
...
impl BundleTransitionBinding {
    /// Build and validate an authenticated bundle transition.
    ///
    /// # Errors
    ///
    /// [`KeystoreRekeyError::BundleBindingInvalid`] when the backend
    /// identifier is empty or exceeds the bound, the epochs are not strictly
    /// increasing, or the two sealed-bundle digests are identical.
    pub fn new(
        bundle_id: [u8; 16],
        ...
        binding
            .validate_detail()
            .map_err(|detail| KeystoreRekeyError::BundleBindingInvalid {
                detail: detail.to_owned(),
            })?;
        Ok(binding)
    }

    fn validate_detail(&self) -> Result<(), &'static str> {
        if self.backend_id.is_empty() {
            return Err("catalog backend identifier is empty");
        }
        if self.backend_id.len() > BACKEND_ID_MAX_BYTES {
            return Err("catalog backend identifier exceeds the byte bound");
        }
        if self.epochs.post <= self.epochs.pre {
            return Err("post-epoch must exceed pre-epoch");
        }
        if self.pre_bundle_b3 == self.post_bundle_b3 {
            return Err("pre- and post-bundle digests must differ");
        }
        Ok(())
    }
}

/// Which recovery mode applies, derived from the authenticated live bundle.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
///
/// Written into a mode-`0600` anonymous inode, synced, and atomically linked
/// next to the database; read back only through a validated descriptor
/// (regular file, current-user owned, link count 1, bounded size). The marker
/// is a **fence**, not the commit point: it makes the broker refuse to open and
/// records the database hashes and authenticated bundle transition for
/// recovery.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct IntentMarker {
    /// Candidate file name inside the staging directory (always
    /// [`CANDIDATE_NAME`] in version 2).
    ...
    pub old_db_b3: [u8; 32],
    /// Authenticated sealed-bundle transition coupled to this database copy.
    pub binding: BundleTransitionBinding,
...
fn hex_decode<const N: usize>(text: &str) -> Option<[u8; N]> {
    if text.len() != N.checked_mul(2)? || !text.is_ascii() {
        return None;
    }
    let mut out = [0u8; N];
...
fn decode_backend_id(text: &str) -> Option<String> {
    let encoded_max = BACKEND_ID_MAX_BYTES.checked_mul(2)?;
    if text.is_empty() || text.len() > encoded_max || !text.len().is_multiple_of(2) {
        return None;
    }
...
impl IntentMarker {
    ...
    fn validate(&self) -> Result<(), KeystoreRekeyError> {
        if self.candidate_name != CANDIDATE_NAME {
            return Err(KeystoreRekeyError::MarkerInvalid {
                detail: format!("candidate name must be `{CANDIDATE_NAME}` in version 2"),
            });
        }
        self.binding
            .validate_detail()
            .map_err(|detail| KeystoreRekeyError::MarkerInvalid {
                detail: detail.to_owned(),
            })?;
        Ok(())
    }

    /// Return the secret-free bundle epochs authenticated by this marker.
    #[must_use]
    pub const fn epochs(&self) -> EpochPair {
        self.binding.epochs
    }

    /// Return the number of records copied into the staged candidate.
    #[must_use]
    pub const fn copied(&self) -> u64 {
        self.binding.copied
    }

    /// Select recovery only for an exact authenticated pre/post bundle tuple.
    ///
    /// # Errors
    ///
    /// [`KeystoreRekeyError::RecoveryUnrecoverable`] when identity, selected
    /// backend, digest, and epoch do not all match one recorded side.
    pub fn phase_for_authenticated_bundle(
        &self,
        bundle_id: [u8; 16],
        backend_id: &str,
        bundle_b3: [u8; 32],
        ...
        let same_affiliation =
            bundle_id == self.binding.bundle_id && backend_id == self.binding.backend_id.as_str();
        if same_affiliation
            && bundle_b3 == self.binding.pre_bundle_b3
            && bundle_epoch == self.binding.epochs.pre
        {
            Ok(RecoveryPhase::RollBack)
        } else if same_affiliation
            && bundle_b3 == self.binding.post_bundle_b3
            && bundle_epoch == self.binding.epochs.post
        {
            Ok(RecoveryPhase::RollForward)
        } else {
            Err(KeystoreRekeyError::RecoveryUnrecoverable {
                detail: "authenticated bundle identity/backend/digest/epoch tuple matches neither \
                         side of the marker transition"
                    .to_owned(),
            })
        }
    }
    ...
    fn serialize(&self) -> String {
        ...
        let _ = writeln!(out, "bundle-id={}", hex_encode(&self.binding.bundle_id));
        let _ = writeln!(
            out,
            "catalog-backend-id-hex={}",
            hex_encode(self.binding.backend_id.as_bytes())
        );
        let _ = writeln!(
            out,
            "pre-bundle-b3={}",
            hex_encode(&self.binding.pre_bundle_b3)
        );
        let _ = writeln!(
            out,
            "post-bundle-b3={}",
            hex_encode(&self.binding.post_bundle_b3)
        );
        let _ = writeln!(out, "pre-epoch={}", self.binding.epochs.pre);
        let _ = writeln!(out, "post-epoch={}", self.binding.epochs.post);
        let _ = writeln!(out, "copied={}", self.binding.copied);
    ...
    fn parse(bytes: &[u8]) -> Result<Self, KeystoreRekeyError> {
        ...
        let candidate_b3 = hex_decode::<32>(field("candidate-b3")?)
            .ok_or_else(|| invalid("bad candidate-b3 hex"))?;
        let old_db_b3 =
            hex_decode::<32>(field("old-db-b3")?).ok_or_else(|| invalid("bad old-db-b3 hex"))?;
        let bundle_id =
            hex_decode::<16>(field("bundle-id")?).ok_or_else(|| invalid("bad bundle-id hex"))?;
        let backend_id = decode_backend_id(field("catalog-backend-id-hex")?)
            .ok_or_else(|| invalid("bad catalog backend identifier encoding or length"))?;
        let pre_bundle_b3 = hex_decode::<32>(field("pre-bundle-b3")?)
            .ok_or_else(|| invalid("bad pre-bundle-b3 hex"))?;
        let post_bundle_b3 = hex_decode::<32>(field("post-bundle-b3")?)
            .ok_or_else(|| invalid("bad post-bundle-b3 hex"))?;
        ...
        let copied = field("copied")?
            .parse::<u64>()
            .map_err(|_| invalid("bad copied count"))?;
        ...
        let marker = Self {
            ...
            candidate_b3,
            old_db_b3,
            binding: BundleTransitionBinding {
                bundle_id,
                backend_id,
                pre_bundle_b3,
                post_bundle_b3,
...
impl StagedCandidate {
    ...
    /// # Errors
    ///
    /// [`KeystoreRekeyError::BundleBindingInvalid`] when `binding` is invalid
    /// or its copied count does not equal the verified staging report.
    pub fn intent_marker(
        &self,
        binding: BundleTransitionBinding,
        ...
        if binding.copied != self.report.copied {
            return Err(KeystoreRekeyError::BundleBindingInvalid {
                detail: "copied count does not match the staged report".to_owned(),
            });
        }
...
trait IntentMarkerOps {
    fn create_unnamed(&self, db_dir: BorrowedFd<'_>) -> Result<File, KeystoreRekeyError>;

    fn write_all(&self, file: &mut File, bytes: &[u8]) -> Result<(), KeystoreRekeyError>;

    fn sync_file(&self, file: &File) -> Result<(), KeystoreRekeyError>;

    fn publish(&self, file: &File, db_dir: BorrowedFd<'_>, name: &str) -> Result<(), Errno>;

    fn sync_dir(&self, db_dir: BorrowedFd<'_>) -> Result<(), KeystoreRekeyError>;
}

struct RealIntentMarkerOps;

impl IntentMarkerOps for RealIntentMarkerOps {
    fn create_unnamed(&self, db_dir: BorrowedFd<'_>) -> Result<File, KeystoreRekeyError> {
        let fd = rustix::fs::openat(
            db_dir,
            ".",
        ...
        .map_err(|errno| fs_err("create anonymous rekey intent marker", errno))?;
        rustix::fs::fchmod(&fd, Mode::from_raw_mode(0o600))
            .map_err(|errno| fs_err("set rekey intent marker permissions", errno))?;
        Ok(File::from(fd))
    }
    ...
    fn publish(&self, file: &File, db_dir: BorrowedFd<'_>, name: &str) -> Result<(), Errno> {
        // `AT_EMPTY_PATH` requires `CAP_DAC_READ_SEARCH` on many kernels.
        // Following this process-owned procfs descriptor is the documented
        // unprivileged `O_TMPFILE` publication path and still names exactly
        // the open anonymous inode.
        let proc_fd = format!("/proc/self/fd/{}", file.as_raw_fd());
        rustix::fs::linkat(
            rustix::fs::CWD,
            proc_fd,
            db_dir,
...
///
/// The complete marker is first written and synced through an anonymous
/// `O_TMPFILE`, then that exact open inode is atomically linked at
/// `<db_name>.rekey-intent`. A failed write or marker sync therefore leaves
/// no visible partial fence. A failed directory sync leaves a complete,
/// parseable fence that normal recovery can resume. The marker is mode
...
/// [`KeystoreRekeyError::MarkerPublished`] when the complete fence exists but
/// its final identity or durability confirmation failed;
/// [`KeystoreRekeyError::Backend`] on a pre-publication filesystem failure.
pub fn write_intent_marker(
    ...
    write_intent_marker_with(db_dir, db_name, marker, lock, &RealIntentMarkerOps)
}

fn write_intent_marker_with<O: IntentMarkerOps>(
    db_dir: BorrowedFd<'_>,
    db_name: &str,
    marker: &IntentMarker,
    lock: &RekeyLock,
    ...
    let mut file = ops.create_unnamed(db_dir)?;
    ops.write_all(&mut file, marker.serialize().as_bytes())?;
    ops.sync_file(&file)?;
    match ops.publish(&file, db_dir, name.as_str()) {
        Ok(()) => {}
        Err(Errno::EXIST) => return Err(KeystoreRekeyError::RekeyInProgress { marker: name }),
        Err(errno) => return Err(fs_err("publish rekey intent marker", errno)),
    }
    let confirmed = (|| {
        let entry = rustix::fs::statat(db_dir, name.as_str(), AtFlags::SYMLINK_NOFOLLOW)
            .map_err(|errno| fs_err("stat published rekey intent marker", errno))?;
        let held = rustix::fs::fstat(&file)
            .map_err(|errno| fs_err("fstat published rekey intent marker", errno))?;
        if entry.st_dev != held.st_dev || entry.st_ino != held.st_ino {
            return Err(KeystoreRekeyError::MarkerInvalid {
                detail: "published marker entry does not match the anonymous inode".to_owned(),
            });
        }
        ops.sync_file(&file)?;
        ops.sync_dir(db_dir)
    })();
    confirmed.map_err(|err| KeystoreRekeyError::MarkerPublished {
        detail: bounded_detail(&err.to_string(), DETAIL_MAX_CHARS),
    })
}
...
pub fn read_intent_marker(
    ...
    Ok(open_intent_marker(db_dir, db_name)?.marker)
}

struct PinnedIntentMarker {
    marker: IntentMarker,
    file: File,
    device: u64,
    inode: u64,
...
fn open_intent_marker(
    db_dir: BorrowedFd<'_>,
    db_name: &str,
) -> Result<PinnedIntentMarker, KeystoreRekeyError> {
    validate_component(db_name)?;
    let name = marker_name(db_name);
    ...
    (&file)
        .take(MARKER_MAX_BYTES.saturating_add(1))
    ...
    let marker = IntentMarker::parse(&bytes)?;
    Ok(PinnedIntentMarker {
        marker,
        file,
        device: stat.st_dev,
        inode: stat.st_ino,
...
fn validate_pinned_marker_entry(
    db_dir: BorrowedFd<'_>,
    db_name: &str,
    pinned: &PinnedIntentMarker,
) -> Result<(), KeystoreRekeyError> {
    let name = marker_name(db_name);
    let entry =
        rustix::fs::statat(db_dir, name.as_str(), AtFlags::SYMLINK_NOFOLLOW).map_err(|errno| {
            if errno == Errno::NOENT {
                KeystoreRekeyError::MarkerMissing {
                    marker: name.clone(),
                }
            } else {
                fs_err("stat rekey intent marker before removal", errno)
            }
        })?;
    let held = rustix::fs::fstat(&pinned.file)
        .map_err(|errno| fs_err("fstat rekey intent marker before removal", errno))?;
    if entry.st_dev != pinned.device
        || entry.st_ino != pinned.inode
        || held.st_dev != pinned.device
        || held.st_ino != pinned.inode
    {
        return Err(KeystoreRekeyError::MarkerInvalid {
            detail: "marker directory entry changed before removal".to_owned(),
        });
    }
    Ok(())
}
...
struct RealTransitionOps;

impl TransitionOps for RealTransitionOps {
    fn checkpoint(
        &self,
        _checkpoint: TransitionCheckpoint,
        _db_dir: BorrowedFd<'_>,
        _db_name: &str,
    ) -> Result<(), KeystoreRekeyError> {
        Ok(())
    }
...
pub fn swap_candidate(
    ...
    swap_candidate_with(db_dir, db_name, staged, lock, &RealTransitionOps)
}

fn swap_candidate_with<O: TransitionOps>(
    db_dir: BorrowedFd<'_>,
    db_name: &str,
    staged: &StagedCandidate,
    lock: &RekeyLock,
    ...
    ops.checkpoint(TransitionCheckpoint::SidecarsUnlinked, db_dir, db_name)?;
    ...
    ops.checkpoint(TransitionCheckpoint::CandidateRenamed, db_dir, db_name)?;
    rustix::fs::fsync(db_dir).map_err(|errno| fs_err("fsync database directory", errno))
}

/// Finish step 5 while keeping the intent marker as the final fence.
///
/// Remove and sync the now-empty staging directory, then revalidate and unlink
/// the pinned marker last and sync again. A crash before fence removal remains
/// recoverable; a crash afterward leaves no stale staging directory.
///
/// # Errors
///
...
pub fn finish_rekey(
    ...
    finish_rekey_with(db_dir, db_name, lock, &RealTransitionOps)
}

fn finish_rekey_with<O: TransitionOps>(
    db_dir: BorrowedFd<'_>,
    db_name: &str,
    lock: &RekeyLock,
    ops: &O,
    ...
    validate_lock_target(lock, db_dir, db_name)?;
    let pinned_marker = open_intent_marker(db_dir, db_name)?;
    let marker = &pinned_marker.marker;
    let live_b3 =
    ...
    rustix::fs::fsync(db_dir).map_err(|errno| fs_err("fsync database directory", errno))?;
    ops.checkpoint(TransitionCheckpoint::StagingRemovalDurable, db_dir, db_name)?;
    validate_pinned_marker_entry(db_dir, db_name, &pinned_marker)?;
    let name = marker_name(db_name);
    match rustix::fs::unlinkat(db_dir, name.as_str(), AtFlags::empty()) {
        Ok(()) => {}
        Err(Errno::NOENT) => return Err(KeystoreRekeyError::MarkerMissing { marker: name }),
...
/// nothing is deleted. On match, candidate and staging cleanup is synced,
/// then the pinned marker is revalidated, removed last, and synced. No DEK
...
pub fn roll_back(
    ...
    marker.validate()?;
    let pinned_marker = open_intent_marker(db_dir, db_name)?;
    if pinned_marker.marker != *marker {
        return Err(KeystoreRekeyError::InvalidPhase {
            detail: "the on-disk intent marker changed before rollback".to_owned(),
        });
    }
    ...
    if let Some(staging) = open_staging_for_recovery(db_dir)? {
        ...
        rustix::fs::fsync(&staging).map_err(|errno| fs_err("fsync staging directory", errno))?;
        drop(staging);
        match rustix::fs::unlinkat(db_dir, STAGING_DIR_NAME, AtFlags::REMOVEDIR) {
            Ok(()) | Err(Errno::NOENT) => {}
            Err(errno) => return Err(fs_err("remove staging directory", errno)),
...
/// the protocol; [`KeystoreRekeyError::InvalidPhase`] when the supplied marker
/// is not the pinned on-disk recovery fence; [`KeystoreRekeyError::Backend`]
...
pub fn roll_forward(
    ...
    if pinned_marker.marker != *marker {
        return Err(KeystoreRekeyError::InvalidPhase {
            detail: "the on-disk intent marker changed before roll-forward".to_owned(),
        });
    }
...
mod tests {
    ...
    const TRANSITION_CHILD_ENV: &str = "BASIL_REKEY_TRANSITION_CHILD";
    const TRANSITION_DIR_ENV: &str = "BASIL_REKEY_TRANSITION_DIR";
    const TRANSITION_TEST_NAME: &str =
        "rekey::tests::transition_checkpoints_survive_sigkill_and_recover";
    const TEST_DB_NAME: &str = "keystore.db";
    const OLD_DATABASE: &[u8] = b"old encrypted database";
    const NEW_DATABASE: &[u8] = b"new encrypted database";
    const SUBSTITUTE_MARKER: &[u8] = b"substituted cleanup marker";
    const TEST_BUNDLE_ID: [u8; 16] = [0x42; 16];
    const TEST_BACKEND_ID: &str = "local-db-keystore";
    const TEST_PRE_BUNDLE_B3: [u8; 32] = [0x31; 32];
    const TEST_POST_BUNDLE_B3: [u8; 32] = [0x32; 32];

    fn sample_binding(copied: u64) -> BundleTransitionBinding {
        BundleTransitionBinding::new(
            TEST_BUNDLE_ID,
            TEST_BACKEND_ID,
            TEST_PRE_BUNDLE_B3,
            TEST_POST_BUNDLE_B3,
            EpochPair { pre: 4, post: 5 },
            copied,
        )
        .unwrap()
    }
    ...
    struct FaultingIntentMarkerOps {
        fault: MarkerFault,
    }

    struct KillAtTransition {
        checkpoint: TransitionCheckpoint,
    }

    impl TransitionOps for KillAtTransition {
        fn checkpoint(
            &self,
            checkpoint: TransitionCheckpoint,
            _db_dir: BorrowedFd<'_>,
            ...
            if checkpoint == self.checkpoint {
                ...
                .expect("SIGKILL transition child");
                loop {
                    std::thread::park();
                }
            }
    ...
    struct SubstituteMarkerAfterCleanup;

    impl TransitionOps for SubstituteMarkerAfterCleanup {
        fn checkpoint(
            &self,
            checkpoint: TransitionCheckpoint,
            db_dir: BorrowedFd<'_>,
            ...
            if checkpoint != TransitionCheckpoint::StagingRemovalDurable {
                return Ok(());
            }
            let name = marker_name(db_name);
            rustix::fs::unlinkat(db_dir, name.as_str(), AtFlags::empty())
                .map_err(|errno| fs_err("replace pinned marker in test", errno))?;
            ...
            .map_err(|errno| fs_err("create substitute marker in test", errno))?;
            let mut replacement = File::from(replacement);
            replacement
                .write_all(SUBSTITUTE_MARKER)
                .map_err(|err| io_err("write substitute marker in test", &err))?;
            replacement
                .sync_all()
                .map_err(|err| io_err("fsync substitute marker in test", &err))?;
            rustix::fs::fsync(db_dir)
                .map_err(|errno| fs_err("fsync substitute marker directory in test", errno))
        }
    }

    impl IntentMarkerOps for FaultingIntentMarkerOps {
        fn create_unnamed(&self, db_dir: BorrowedFd<'_>) -> Result<File, KeystoreRekeyError> {
            RealIntentMarkerOps.create_unnamed(db_dir)
        }

        fn write_all(&self, file: &mut File, bytes: &[u8]) -> Result<(), KeystoreRekeyError> {
            if let MarkerFault::Write(errno) = self.fault {
                let split = bytes.len() / 2;
                RealIntentMarkerOps.write_all(file, bytes.get(..split).unwrap_or_default())?;
                return Err(fs_err("injected rekey intent marker write", errno));
            }
            RealIntentMarkerOps.write_all(file, bytes)
        }

        fn sync_file(&self, file: &File) -> Result<(), KeystoreRekeyError> {
            if matches!(self.fault, MarkerFault::FileSync) {
                return Err(fs_err("injected rekey intent marker fsync", Errno::IO));
            }
            RealIntentMarkerOps.sync_file(file)
        }

        fn publish(&self, file: &File, db_dir: BorrowedFd<'_>, name: &str) -> Result<(), Errno> {
            match self.fault {
                MarkerFault::Publish(errno) => Err(errno),
                MarkerFault::Substitute => {
                    let replacement = rustix::fs::openat(
                        db_dir,
                        name,
                        OFlags::WRONLY
                    ...
                    let mut replacement = File::from(replacement);
                    replacement
                        .write_all(b"substituted marker")
                        .map_err(|err| Errno::from_io_error(&err).unwrap_or(Errno::IO))?;
                    replacement
                        .sync_all()
                        .map_err(|err| Errno::from_io_error(&err).unwrap_or(Errno::IO))?;
                    drop(replacement);
                    RealIntentMarkerOps.publish(file, db_dir, name)
                }
                MarkerFault::Write(_) | MarkerFault::FileSync | MarkerFault::DirectorySync => {
                    RealIntentMarkerOps.publish(file, db_dir, name)
                }
            }
        }
        ...
        fn sync_dir(&self, db_dir: BorrowedFd<'_>) -> Result<(), KeystoreRekeyError> {
            if matches!(self.fault, MarkerFault::DirectorySync) {
                return Err(fs_err("injected database directory fsync", Errno::IO));
            }
            RealIntentMarkerOps.sync_dir(db_dir)
        }
    }

    fn marker_test_target(stem: &str) -> (std::path::PathBuf, OwnedFd, RekeyLock) {
        let dir = std::env::temp_dir().join(format!(
            "basil-rekey-marker-{stem}-{}-{}",
            std::process::id(),
        ...
        std::fs::write(dir.join("keystore.db"), b"stable target inode").unwrap();
    ...
    fn transition_test_dir(stem: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "basil-rekey-transition-{stem}-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .map_or(0, |duration| duration.as_nanos())
        ...
        std::fs::create_dir(&dir).unwrap();
        std::fs::write(dir.join(TEST_DB_NAME), OLD_DATABASE).unwrap();
        let staging = dir.join(STAGING_DIR_NAME);
        std::fs::create_dir(&staging).unwrap();
        std::fs::write(staging.join(CANDIDATE_NAME), NEW_DATABASE).unwrap();
        for suffix in SIDECAR_SUFFIXES {
            std::fs::write(dir.join(format!("{TEST_DB_NAME}{suffix}")), b"old sidecar").unwrap();
        }
        dir
    }

    fn open_test_dir(dir: &Path) -> OwnedFd {
        rustix::fs::open(
            dir,
            OFlags::RDONLY | OFlags::DIRECTORY | OFlags::NOFOLLOW | OFlags::CLOEXEC,
            Mode::empty(),
    ...
    fn directory_fd_matches_path(directory: BorrowedFd<'_>, path: &Path) -> bool {
        let Ok(fd_stat) = rustix::fs::fstat(directory) else {
            return false;
        };
        let Ok(path_stat) = rustix::fs::stat(path) else {
            return false;
        };
    ...
    impl DatabaseDirectorySync for FailOnceDirectorySync {
        fn sync(
            &self,
            directory: BorrowedFd<'_>,
            operation: &'static str,
        ) -> Result<(), KeystoreRekeyError> {
            if directory_fd_matches_path(directory, &self.database_directory)
    ...
    impl DatabaseDirectorySync for BlockingDirectorySync {
        fn sync(
            &self,
            directory: BorrowedFd<'_>,
            operation: &'static str,
                ...
                && !self.blocked.swap(true, Ordering::SeqCst)
            {
                ...
                self.entered
                    .send(())
                    .map_err(|_| fs_err("signal database directory fsync", Errno::CANCELED))?;
                self.release
                    .recv_timeout(Duration::from_secs(5))
                    .map_err(|_| {
                        fs_err("await database directory fsync release", Errno::TIMEDOUT)
                    })?;
            }
            rustix::fs::fsync(directory).map_err(|errno| fs_err(operation, errno))
        }
    ...
    fn open_test_staged(db_dir: BorrowedFd<'_>) -> StagedCandidate {
        let staging_dir = rustix::fs::openat(
            db_dir,
            STAGING_DIR_NAME,
            OFlags::RDONLY | OFlags::DIRECTORY | OFlags::NOFOLLOW | OFlags::CLOEXEC,
            Mode::empty(),
        ...
        .unwrap();
        let candidate_b3 = blake3_of_fd(candidate_fd.as_fd()).unwrap();
        let old_db_b3 = blake3_of_entry(db_dir, TEST_DB_NAME).unwrap().unwrap();
        StagedCandidate {
            staging_dir,
            candidate_fd,
            report: RekeyReport { copied: 1 },
            candidate_b3,
            old_db_b3,
            target: target_for(db_dir, TEST_DB_NAME).unwrap(),
        }
    }

    fn prepare_transition(stem: &str, swap_complete: bool) -> PathBuf {
        let dir = transition_test_dir(stem);
        let db_dir = open_test_dir(&dir);
        let lock = RekeyLock::acquire_exclusive(db_dir.as_fd(), TEST_DB_NAME).unwrap();
        let staged = open_test_staged(db_dir.as_fd());
        let marker = staged.intent_marker(sample_binding(1)).unwrap();
        write_intent_marker(db_dir.as_fd(), TEST_DB_NAME, &marker, &lock).unwrap();
        if swap_complete {
            swap_candidate(db_dir.as_fd(), TEST_DB_NAME, &staged, &lock).unwrap();
        }
        drop(staged);
        drop(lock);
        drop(db_dir);
    ...
    fn run_transition_child_if_requested() -> bool {
        let Ok(role) = std::env::var(TRANSITION_CHILD_ENV) else {
            return false;
        };
        let dir = PathBuf::from(std::env::var_os(TRANSITION_DIR_ENV).expect("transition dir"));
        let checkpoint = match role.as_str() {
            "sidecars" => TransitionCheckpoint::SidecarsUnlinked,
            "rename" => TransitionCheckpoint::CandidateRenamed,
            "cleanup" => TransitionCheckpoint::StagingRemovalDurable,
            other => panic!("unknown transition child role: {other}"),
        };
        let ops = KillAtTransition { checkpoint };
        let db_dir = open_test_dir(&dir);
        let lock = RekeyLock::acquire_exclusive(db_dir.as_fd(), TEST_DB_NAME).unwrap();
        let result = if checkpoint == TransitionCheckpoint::StagingRemovalDurable {
            finish_rekey_with(db_dir.as_fd(), TEST_DB_NAME, &lock, &ops)
        } else {
            let staged = open_test_staged(db_dir.as_fd());
            swap_candidate_with(db_dir.as_fd(), TEST_DB_NAME, &staged, &lock, &ops)
        };
        panic!("transition child escaped checkpoint without SIGKILL: {result:?}");
    }
    ...
    #[test]
    fn visible_directory_after_sync_failure_is_resynced_before_guard_progress() {
        let root = std::env::temp_dir().join(format!(
            "basil-db-parent-sync-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .map_or(0, |duration| duration.as_nanos())
        ...
        let database_path = database_directory.join(TEST_DB_NAME);

        let failing_sync = FailOnceDirectorySync {
            database_directory: database_directory.clone(),
            injected: AtomicBool::new(false),
        };
        let error = guard_store_open_with(&database_path, &failing_sync).unwrap_err();
        assert!(matches!(error, KeystoreRekeyError::Backend { .. }));
        assert!(failing_sync.injected.load(Ordering::SeqCst));
        assert!(database_directory.is_dir());
        assert!(!database_path.exists());
        assert!(
            !database_directory
                .join(format!("{TEST_DB_NAME}{LOCK_SUFFIX}"))
                .exists()
        );
        ...
        let follower_attempts = Arc::clone(&sync_attempts);
        let follower_directory = database_directory.clone();
        let follower_path = database_path.clone();
        let follower = std::thread::spawn(move || {
            let retry_sync = BlockingDirectorySync {
                database_directory: follower_directory,
                entered: entered_tx,
                release: release_rx,
                blocked: AtomicBool::new(false),
                attempts: follower_attempts,
            };
            let result = guard_store_open_with(&follower_path, &retry_sync);
            done_tx.send(result).unwrap();
        });

        entered_rx
            .recv_timeout(Duration::from_secs(5))
            .expect("retry reached the visible directory fsync");
        assert!(matches!(
        ...
        let lock = done_rx
            .recv_timeout(Duration::from_secs(5))
            .expect("retry completed after directory durability")
            .expect("retry guard succeeded");
        assert_eq!(sync_attempts.load(Ordering::SeqCst), 1);
        assert!(
            database_directory
                .join(format!("{TEST_DB_NAME}{LOCK_SUFFIX}"))
                .exists()
    ...
    #[test]
    fn bundle_binding_bounds_and_staged_count_fail_closed() {
        for binding in [
            BundleTransitionBinding::new(
                TEST_BUNDLE_ID,
                "",
            ...
            BundleTransitionBinding::new(
                TEST_BUNDLE_ID,
                "x".repeat(BACKEND_ID_MAX_BYTES + 1),
                TEST_PRE_BUNDLE_B3,
                TEST_POST_BUNDLE_B3,
                EpochPair { pre: 4, post: 5 },
                1,
            ...
            BundleTransitionBinding::new(
                ...
                TEST_PRE_BUNDLE_B3,
                TEST_POST_BUNDLE_B3,
                EpochPair { pre: 5, post: 5 },
                1,
            ),
        ] {
            assert!(matches!(
                binding,
                Err(KeystoreRekeyError::BundleBindingInvalid { .. })
            ));
        }

        let dir = transition_test_dir("copied-count");
        let db_dir = open_test_dir(&dir);
        let staged = open_test_staged(db_dir.as_fd());
        let err = staged.intent_marker(sample_binding(2)).unwrap_err();
        assert!(matches!(
            err,
            KeystoreRekeyError::BundleBindingInvalid { .. }
        ));
        assert!(!marker_present(db_dir.as_fd(), TEST_DB_NAME).unwrap());
        drop(staged);
        drop(db_dir);
    ...
    #[test]
    fn marker_write_and_file_sync_faults_never_publish_partial_fences() {
        for (label, fault) in [
            ("enospc", MarkerFault::Write(Errno::NOSPC)),
            ("edquot", MarkerFault::Write(Errno::DQUOT)),
            ("file-sync", MarkerFault::FileSync),
            ("cross-device", MarkerFault::Publish(Errno::XDEV)),
        ] {
            let (dir, db_dir, lock) = marker_test_target(label);
            let ops = FaultingIntentMarkerOps { fault };
            let result = write_intent_marker_with(
                db_dir.as_fd(),
                "keystore.db",
                &sample_marker(),
                &lock,
            ...
            assert!(result.is_err(), "{label} fault must fail");
            assert!(
                !marker_present(db_dir.as_fd(), "keystore.db").unwrap(),
                "{label} fault must not publish a partial marker"
            );
            write_intent_marker(db_dir.as_fd(), "keystore.db", &sample_marker(), &lock).unwrap();
            assert_eq!(
                read_intent_marker(db_dir.as_fd(), "keystore.db").unwrap(),
                sample_marker(),
                "{label} failure must be retryable with the same staged intent"
            );
            drop(lock);
            drop(db_dir);
            std::fs::remove_dir_all(dir).unwrap();
    ...
    #[test]
    fn marker_publication_never_erases_a_substituted_entry() {
        let (dir, db_dir, lock) = marker_test_target("substitution");
        let ops = FaultingIntentMarkerOps {
            fault: MarkerFault::Substitute,
        };
        let err =
            write_intent_marker_with(db_dir.as_fd(), "keystore.db", &sample_marker(), &lock, &ops)
                .unwrap_err();
        assert!(matches!(err, KeystoreRekeyError::RekeyInProgress { .. }));
        assert_eq!(
            std::fs::read(dir.join("keystore.db.rekey-intent")).unwrap(),
            b"substituted marker"
        );
        drop(lock);
        drop(db_dir);
        std::fs::remove_dir_all(dir).unwrap();
    ...
    #[test]
    fn directory_sync_failure_leaves_a_complete_recoverable_marker() {
        let (dir, db_dir, lock) = marker_test_target("directory-sync");
        let ops = FaultingIntentMarkerOps {
            fault: MarkerFault::DirectorySync,
        };
        let marker = sample_marker();
        let err = write_intent_marker_with(db_dir.as_fd(), "keystore.db", &marker, &lock, &ops)
            .unwrap_err();
        assert!(matches!(err, KeystoreRekeyError::MarkerPublished { .. }));
    ...
    #[test]
    fn transition_checkpoints_survive_sigkill_and_recover() {
        if run_transition_child_if_requested() {
            return;
        }
        for (role, checkpoint, expected) in [
            (
                "sidecars",
                TransitionCheckpoint::SidecarsUnlinked,
                RecoveryOutcome::ResumedSwap,
            ),
            (
                "rename",
                TransitionCheckpoint::CandidateRenamed,
                RecoveryOutcome::SwapAlreadyComplete,
            ),
            (
                "cleanup",
                TransitionCheckpoint::StagingRemovalDurable,
                RecoveryOutcome::SwapAlreadyComplete,
            ),
        ] {
            let dir = prepare_transition(
                role,
                checkpoint == TransitionCheckpoint::StagingRemovalDurable,
            );
            let status = Command::new(std::env::current_exe().unwrap())
                .args(["--exact", TRANSITION_TEST_NAME, "--nocapture"])
                .env(TRANSITION_CHILD_ENV, role)
                .env(TRANSITION_DIR_ENV, &dir)
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .status()
            ...
            assert_eq!(
                ...
                "{role} child must stop at the exact checkpoint"
            );

            if checkpoint == TransitionCheckpoint::SidecarsUnlinked {
                assert_eq!(std::fs::read(dir.join(TEST_DB_NAME)).unwrap(), OLD_DATABASE);
                assert!(dir.join(STAGING_DIR_NAME).join(CANDIDATE_NAME).exists());
            } else {
                assert_eq!(std::fs::read(dir.join(TEST_DB_NAME)).unwrap(), NEW_DATABASE);
                assert!(!dir.join(STAGING_DIR_NAME).join(CANDIDATE_NAME).exists());
            }
            for suffix in SIDECAR_SUFFIXES {
                assert!(!dir.join(format!("{TEST_DB_NAME}{suffix}")).exists());
            }
            assert!(dir.join(format!("{TEST_DB_NAME}{MARKER_SUFFIX}")).exists());

            let db_dir = open_test_dir(&dir);
            let lock = RekeyLock::acquire_exclusive(db_dir.as_fd(), TEST_DB_NAME).unwrap();
            let marker = read_intent_marker(db_dir.as_fd(), TEST_DB_NAME).unwrap();
            assert_eq!(
                roll_forward(db_dir.as_fd(), TEST_DB_NAME, &marker, &lock).unwrap(),
                expected
            );
            assert_eq!(std::fs::read(dir.join(TEST_DB_NAME)).unwrap(), NEW_DATABASE);
            assert!(!dir.join(STAGING_DIR_NAME).exists());
            assert!(!dir.join(format!("{TEST_DB_NAME}{MARKER_SUFFIX}")).exists());
            drop(lock);
            drop(db_dir);
            std::fs::remove_dir_all(dir).unwrap();
    ...
    #[test]
    fn finish_preserves_a_marker_substituted_after_durable_cleanup() {
        let dir = prepare_transition("cleanup-substitute", true);
        let db_dir = open_test_dir(&dir);
        let lock = RekeyLock::acquire_exclusive(db_dir.as_fd(), TEST_DB_NAME).unwrap();
        let err = finish_rekey_with(
            db_dir.as_fd(),
            TEST_DB_NAME,
            &lock,
            &SubstituteMarkerAfterCleanup,
        )
        .unwrap_err();
        assert!(matches!(err, KeystoreRekeyError::MarkerInvalid { .. }));
        assert_eq!(
            std::fs::read(dir.join(format!("{TEST_DB_NAME}{MARKER_SUFFIX}"))).unwrap(),
            SUBSTITUTE_MARKER
        );
        assert!(!dir.join(STAGING_DIR_NAME).exists());
        drop(lock);
    ...
    fn marker_parse_rejects_confusion_and_tampering() {
        ...
        let cases: Vec<String> = vec![
            good.replace(MARKER_MAGIC, "basil-rekey-intent-v1"),
            ...
            good.replace("bundle-id=4242", "bundle-id=AB42"),
            good.replace("catalog-backend-id-hex=", "catalog-backend-id-hex=zz"),
            {
                // Reordered fields.
    ...
    #[test]
    fn phase_derivation_requires_the_exact_authenticated_bundle_tuple() {
        let marker = sample_marker();
        assert_eq!(marker.epochs(), EpochPair { pre: 4, post: 5 });
        assert_eq!(marker.copied(), 2);
        assert_eq!(
            marker
                .phase_for_authenticated_bundle(
                    TEST_BUNDLE_ID,
                    TEST_BACKEND_ID,
                    TEST_PRE_BUNDLE_B3,
                    4,
        ...
        for result in [
            ...
            marker.phase_for_authenticated_bundle(TEST_BUNDLE_ID, TEST_BACKEND_ID, [0xee; 32], 4),
            marker.phase_for_authenticated_bundle(TEST_BUNDLE_ID, TEST_BACKEND_ID, [0xef; 32], 5),
            marker.phase_for_authenticated_bundle(TEST_BUNDLE_ID, TEST_BACKEND_ID, [0xed; 32], 7),
        ] {
            assert!(matches!(
                result,
                Err(KeystoreRekeyError::RecoveryUnrecoverable { .. })
    ...
    #[test]
    fn sensitive_dek_copies_into_the_secret_container() {
        let dek = SensitiveDek::from_raw(Zeroizing::new([0xab; 32]));
        let secret = dek.to_secret_array();
        assert_eq!(secret.expose_secret(), &[0xab; 32]);
        assert_eq!(format!("{dek:?}"), "SensitiveDek(<redacted>)");
    }
    ...
    fn hex_helpers_round_trip() {
        ...
        assert!(hex_decode::<32>("zz").is_none());
        assert!(hex_decode::<32>(&"g".repeat(64)).is_none());
        assert!(hex_decode::<32>(&"A".repeat(64)).is_none());
    }
}
