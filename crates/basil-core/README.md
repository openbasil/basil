...
| `core::ci_federation`                                                         | Closed CI OIDC provider catalog (GitHub Actions): typed non-overlapping rules, strict `RS256` claim verification with the rule clock skew as the single time authority, generation-owned JWKS caches with bounded cooldown-gated refresh and a positive freshness window (`max_age` + `stale_if_error` bound how long a provider-revoked key keeps verifying), proof-key thumbprint audiences, and keyed token/`jti` correlation digests.                 |
