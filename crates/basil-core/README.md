...
The Basil daemon as a library: broker core, backend adapters, gRPC services, transport, and the
offline operator command implementations. The shipped [`basil-bin`](../basil-bin) binary is a thin
parser over this crate; embedders and the integration tests link it directly.
...
talk to it with the [`basil`](../basil-client) client crate. Reach for `basil-core` when you are
