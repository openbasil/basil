...
//! path (for example Compose long-syntax bind mounts, which deliberately disable
//! host-path auto-creation) keeps failing closed until the operator regenerates
//! it and recreates the affected workloads. The [`RewireLedger`] records every
...
mod tests {
    ...
    fn updates_classify_removal_path_change_and_ignore_additions() {
        let current = set(&[
            ...
            ("workloads", ListenerType::Container, "/run/basil/w.sock"),
        ]);
        let candidate = set(&[
            ("host", ListenerType::Host, "/run/basil/host-v2.sock"),
            ("extra", ListenerType::Container, "/run/basil/extra.sock"),
        ]);
        ...
        assert!(updates.iter().any(|update| matches!(
            update,
            RewireUpdate::Resolve { listener } if listener == "workloads"
        )));
    ...
    fn mode_only_reconfiguration_and_pure_addition_record_nothing() {
        ...
        let candidate = set(&[
            ("host", ListenerType::Host, "/run/basil/host.sock"),
            ("aux", ListenerType::Container, "/run/basil/aux.sock"),
        ]);
