...
/// Default accepted-transport ceiling across courier listeners.
///
/// Compiled strictly below [`DEFAULT_MAX_CONNECTIONS`] so couriers can never
/// occupy the whole global ceiling: the host/operator surface always retains
/// `DEFAULT_MAX_CONNECTIONS - DEFAULT_MAX_CONNECTIONS_COURIER`
/// admittable connections, whatever the non-host side does (`basil-22pt`).
pub const DEFAULT_MAX_CONNECTIONS_COURIER: usize = 3072;
...
pub struct ConnectionCeilings {
    ...
    /// Aggregate bound across every courier listener.
    pub maximum_courier: usize,
}
...
impl ConnectionCeilings {
    ...
    pub const fn compiled() -> Self {
        Self {
            ...
            maximum_courier: DEFAULT_MAX_CONNECTIONS_COURIER,
        }
    }
    ...
    pub const fn maximum_for_type(&self, listener_type: ListenerType) -> usize {
        match listener_type {
            ...
            ListenerType::Courier => self.maximum_courier,
        }
    }
...
impl TypeCounts {
    ...
    ///
    /// Admission count for the courier resource class.
...
impl ConnectionIdentity {
    ...
    /// Typed systemd identity, when the domain supplies one.
    #[must_use]
...
mod tests {
    ...
    fn limits_must_be_nonzero_and_coherent() {
        ...
        for broken in [
            ...
            ConnectionCeilings {
                maximum_courier: DEFAULT_MAX_CONNECTIONS + 1,
                ..ConnectionCeilings::compiled()
            },
        ] {
            assert!(matches!(
    ...
    /// The compiled defaults guarantee host/operator headroom: courier
    ...
    fn compiled_ceilings_reserve_host_headroom() {
        ...
        assert!(compiled.maximum_courier < compiled.maximum);
        assert!(
            compiled.maximum - compiled.maximum_courier >= DEFAULT_MAX_CONNECTIONS_HOST,
            "courier exhaustion must leave a full host ceiling admittable"
        );
        assert_eq!(
    ...
    async fn listener_type_ceiling_is_shared_typed_and_released() {
        ...
        let first = registry
            .register(stream, "c1", ListenerType::Courier)
            .expect("first courier connection");
        let (stream, _peer_b) = pair();
        let second = registry
            .register(stream, "c2", ListenerType::Courier)
            .expect("second courier listener shares the type ceiling");
        assert_eq!(registry.active_for_type(ListenerType::Courier), 2);

        // A third courier listener name is still bounded by the type.
        let (stream, _peer_c) = pair();
        assert!(matches!(
            registry.register(stream, "c3", ListenerType::Courier),
            Err(ConnectionRegistryError::ListenerTypeLimit(
                ListenerType::Courier
            ))
        ));

        // Courier exhaustion never starves the host type.
        let (stream, _peer_d) = pair();
        let host = registry
            .register(stream, "host", ListenerType::Host)
            .expect("host headroom survives courier exhaustion");
        ...
        let third = registry
            .register(stream, "c3", ListenerType::Courier)
    ...
    async fn capacity_is_enforced_before_tracking_and_ids_never_reuse() {
        ...
        let courier = registry
            .register(stream, "courier", ListenerType::Courier)
            .expect("second listener fits global capacity");
        let (stream, _peer) = pair();
        assert!(matches!(
            registry.register(stream, "other", ListenerType::Courier),
    ...
    async fn typed_cancellation_is_atomic_deduplicated_and_excludes_caller() {
        ...
        let actor = AuthenticatedActor {
            domain: AuthorizationDomain::SystemdUnit,
            subject: "svc.api".to_string(),
            workload_identity: Some(WorkloadIdentity::Systemd {
                unit: "api.service".to_string(),
                manager_user: Some(1_000),
            }),
        ...
        let selector = ConnectionSelector::Systemd {
            unit: "api.service".to_string(),
            manager_user: Some(1_000),
        };
