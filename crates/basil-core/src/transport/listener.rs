    ...
    fn named_listener_set_is_typed_bounded_and_ordered() {
        let listeners = ListenerConfigSet::resolve(
            BTreeMap::from([
                (
                    "courier".to_string(),
                    input(ListenerType::Courier, "/run/basil/courier/agent.sock"),
                ),
                (
                    "control".to_string(),
        ...
        assert_eq!(
            listeners.get("courier").map(ListenerConfig::listener_type),
            Some(ListenerType::Courier)
        );
    }
    ...
    fn invalid_names_paths_duplicates_modes_and_missing_host_fail() {
        ...
        let duplicate = ListenerConfigSet::resolve(
            BTreeMap::from([
                ...
                (
                    "courier".to_string(),
                    input(ListenerType::Courier, "/run/basil/agent.sock"),
                ),
            ]),
            LegacyListenerConfig::default(),
        )
        ...
        assert_eq!(
            ListenerConfigSet::resolve(
                BTreeMap::from([(
                    "courier".to_string(),
                    input(ListenerType::Courier, "/run/basil/courier.sock"),
                )]),
                LegacyListenerConfig::default(),
            ),
