...
//! the catalog, policy, and sealed bundle. This module
...
impl ConfigOverride {
    ...
    fn is_document_path(&self) -> bool {
        self.path.starts_with("catalog.") || self.path.starts_with("policy.")
    }
}
...
pub(crate) fn load_bootstrap_with_trace_collector(
    ...
    build_loaded_bootstrap(path, &mut value, overrides)
}

pub(crate) fn build_loaded_bootstrap(
    path: PathBuf,
    ...
    for config_override in overrides.iter().filter(|item| item.is_source_path()) {
        apply_source_override(value, config_override)?;
    ...
    {
        apply_scalar_override(value, config_override)?;
        provenance.push(OverrideProvenance {
            path: config_override.path.clone(),
            masked_source: path.clone(),
        });
...
/// discriminator, or fails catalog/policy semantic validation.
...
pub(crate) fn load_direct_documents_with_context(
    ...
    let result = (|| {
        let (catalog_bytes, catalog_trace) =
            read_configuration_source("catalog", None, &sources.catalog).map_err(|source| {
                ConfigurationError::ReadDocument {
                    slot: "catalog".to_string(),
                    path: sources.catalog.clone(),
                    source,
                }
            })?;
        traces.push(catalog_trace);
        let (policy_bytes, policy_trace) =
            read_configuration_source("policy", None, &sources.policy).map_err(|source| {
                ConfigurationError::ReadDocument {
                    slot: "policy".to_string(),
                    path: sources.policy.clone(),
                    source,
                }
            })?;
        traces.push(policy_trace);
        let catalog_json = std::str::from_utf8(&catalog_bytes).map_err(|source| {
            ConfigurationError::DecodeDocument {
                slot: "catalog".to_string(),
                path: sources.catalog.clone(),
                source,
            }
...
fn validate_bootstrap_header(value: &toml::Value) -> Result<(), ConfigurationError> {
    ...
    {
        return Err(ConfigurationError::InvalidCorpus(
            "schema-3 bootstrap source paths belong under `[import]`".to_string(),
        ));
    }
...
fn extract_sources(
    ...
    if let Some(name) = imports
        .keys()
        .find(|name| !matches!(name.as_str(), "catalog" | "policy" | "bundle"))
    {
        return Err(ConfigurationError::InvalidCorpus(format!(
...
mod tests {
    ...
    fn source_trace_hashes_the_exact_bytes_read() {
        ...
        let (bytes, trace) =
            read_configuration_source("catalog", Some("web"), &path).expect("read source");

        assert_eq!(bytes, b"abc");
        assert_eq!(trace.slot, "catalog");
...
"#,
        ...
        let structural = [ConfigOverride::parse("import.workload=x").expect("parse")];
