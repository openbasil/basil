...
//! courier-observed source, and globally, and exactly-once consumption. The
//! table is deliberately safe to lose: restart or eviction invalidates
...
pub struct ChallengeTableConfig {
    ...
    /// Per courier-observed-source issuance token bucket.
...
pub struct ChallengeTable {
    ...
    source_buckets: HashMap<String, TokenBucket>,
}
...
impl ChallengeTable {
    ...
    /// the courier-observed rate-limit partition key; `None` means the
    /// caller reached the broker locally without a courier. Capacity and
    /// instance-ID entropy are checked before any rate-limit token is
    /// consumed, so a capacity or entropy decline does not additionally
    /// rate-limit the caller's (or anyone's) later retries.
    ///
    /// # Errors
    ...
    pub fn issue(
        ...
        let Some(instance_id) = self.ensure_instance_id() else {
        ...
        if !self
            .global_bucket
            .try_take(self.config.global_rate, now_unix)
        {
            return Err(IssueDecline::RateLimited);
        }
        if let Some(source) = source
            && !take_partition_token(
        ...
        if !take_partition_token(
            &mut self.jkt_buckets,
            jkt,
    ...
    fn ensure_instance_id(&mut self) -> Option<[u8; INSTANCE_ID_LEN]> {
        if self.instance_id.is_none() {
            self.instance_id = random_bytes::<INSTANCE_ID_LEN>();
        }
        self.instance_id
    }
    ...
    fn unused_suffix(&self) -> Option<[u8; CHALLENGE_SUFFIX_LEN]> {
        for _ in 0_u8..4 {
            let suffix = random_bytes::<CHALLENGE_SUFFIX_LEN>()?;
    ...
    fn purge_expired(&mut self, now_unix: i64) {
        let expired: Vec<[u8; CHALLENGE_SUFFIX_LEN]> = self
            .records
            .iter()
...
/// Take one token from the bucket for `key`, creating the partition if it can
/// be tracked. When the partition map is full, idle partitions are evicted
/// first; if none can be evicted the request is declined (metering never
/// silently disables under pressure).
fn take_partition_token<K: std::hash::Hash + Eq + Clone>(
    ...
    if let Some(bucket) = buckets.get_mut(&key) {
        return bucket.try_take(config, now_unix);
    }
    if buckets.len() >= tracked_partitions {
        buckets.retain(|_, bucket| !bucket.is_idle(config, now_unix));
    }
    if buckets.len() >= tracked_partitions {
        return false;
    }
    let mut bucket = TokenBucket::new(config, now_unix);
    let taken = bucket.try_take(config, now_unix);
...
mod tests {
    ...
    fn issued_challenge_is_instance_prefixed_with_bounded_expiry() {
        let mut table = table();
        let issued = table.issue(jkt(1), None, GEN, NOW).expect("issues");
        assert_eq!(issued.expires_at_unix, NOW + MAX_CHALLENGE_TTL_SECS);
        let again = table.issue(jkt(1), None, GEN, NOW).expect("issues");
        assert_eq!(
    ...
    fn consume_is_exactly_once() {
        let mut table = table();
        let issued = table.issue(jkt(1), None, GEN, NOW).unwrap();
    ...
    fn restart_invalidates_outstanding_challenges_via_instance_prefix() {
        ...
        let issued = before.issue(jkt(1), None, GEN, NOW).unwrap();
        let mut after = table();
        assert_eq!(
    ...
    fn per_jkt_capacity_is_eight_and_scoped_to_the_thumbprint() {
        ...
        for _ in 0..MAX_OUTSTANDING_PER_JKT {
            table.issue(jkt(1), None, GEN, NOW).expect("under the cap");
        }
        assert_eq!(
            table.issue(jkt(1), None, GEN, NOW),
            Err(IssueDecline::CapacityExhausted)
        );
        // A different thumbprint is unaffected.
        table
            .issue(jkt(2), None, GEN, NOW)
    ...
    fn global_capacity_declines_issuance_but_never_consumption() {
        ...
        for seed in 0..4_u8 {
            issued.push((seed, table.issue(jkt(seed), None, GEN, NOW).unwrap()));
        }
        assert_eq!(
            table.issue(jkt(9), None, GEN, NOW),
            Err(IssueDecline::CapacityExhausted)
        );
    ...
    fn expired_records_are_dropped_before_declining_issuance() {
        ...
        table.issue(jkt(1), None, GEN, NOW).unwrap();
        table.issue(jkt(2), None, GEN, NOW).unwrap();
    ...
    fn capacity_declines_do_not_consume_rate_tokens() {
        ...
        let mut issued = Vec::new();
        for _ in 0..MAX_OUTSTANDING_PER_JKT {
            issued.push(table.issue(jkt(1), None, GEN, NOW).expect("under the cap"));
        }
        for _ in 0..3 {
            assert_eq!(
                table.issue(jkt(1), None, GEN, NOW),
    ...
    fn per_jkt_issuance_rate_limit_declines_then_refills() {
        ...
        table.issue(jkt(1), None, GEN, NOW + 1).expect("refilled");
    }
    ...
    fn per_source_rate_limit_partitions_by_courier_observed_source() {
        ...
        table.issue(jkt(1), Some("10.0.0.1"), GEN, NOW).unwrap();
        assert_eq!(
            table.issue(jkt(2), Some("10.0.0.1"), GEN, NOW),
            Err(IssueDecline::RateLimited)
        );
        table
            .issue(jkt(2), Some("10.0.0.2"), GEN, NOW)
            .expect("distinct source issues");
        table
            .issue(jkt(3), None, GEN, NOW)
            .expect("local callers bypass the courier partition only");
    }
    ...
    fn global_rate_limit_declines_all_issuance() {
        ...
        table.issue(jkt(1), None, GEN, NOW).unwrap();
        assert_eq!(
            table.issue(jkt(2), None, GEN, NOW),
            Err(IssueDecline::RateLimited)
        );
    }
    ...
    fn untrackable_new_partition_declines_instead_of_unmetered_service() {
        ...
        table.issue(jkt(1), Some("a"), GEN, NOW).unwrap();
        // "a" holds a non-idle bucket; a second source cannot be tracked.
        assert_eq!(
            table.issue(jkt(2), Some("b"), GEN, NOW),
            Err(IssueDecline::RateLimited)
        );
        ...
        roomy.issue(jkt(1), Some("a"), GEN, NOW).unwrap();
        roomy
            .issue(jkt(2), Some("b"), GEN, NOW + 5)
