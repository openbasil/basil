...
pub fn load(
    ...
    let catalog_json = normalize_legacy_document(catalog_json, LegacyDocument::Catalog)?;
    let policy_json = normalize_legacy_document(policy_json, LegacyDocument::Policy)?;
    let catalog: Catalog =
        serde_json::from_str(catalog_json.as_ref()).map_err(|source| LoadError::Json {
            what: "catalog",
            source,
        })?;
    let raw_policy: RawPolicy =
        serde_json::from_str(policy_json.as_ref()).map_err(|source| LoadError::Json {
            what: "policy",
            source,
        })?;
...
fn normalize_legacy_document(
...
) -> Result<Cow<'_, str>, LoadError> {
    let what = match document {
        LegacyDocument::Catalog => "catalog",
        LegacyDocument::Policy => "policy",
    };
    let value = serde_json::from_str::<serde_json::Value>(raw)
        .map_err(|source| LoadError::Json { what, source })?;
    let Some(object) = value.as_object() else {
        return Ok(Cow::Borrowed(raw));
    };
    let (legacy_version, discriminator, required_shape) = match document {
        LegacyDocument::Catalog => (
            1,
            "catalog",
            object
                .get("backends")
    ...
    let is_unambiguous_legacy = object.get("schema").is_none()
        && object
            .get("schemaVersion")
            .and_then(serde_json::Value::as_u64)
            == Some(legacy_version)
        && required_shape;
    if is_unambiguous_legacy {
        return rewrite_legacy_schema_version(raw, legacy_version, discriminator)
            .map(Cow::Owned)
            .ok_or_else(|| LoadError::Json {
                what,
                source: serde_json::Error::io(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    "could not locate the top-level legacy schema discriminator",
                )),
            });
    }
    Ok(Cow::Borrowed(raw))
...
fn rewrite_legacy_schema_version(
    raw: &str,
    ...
    for (index, character) in raw.char_indices() {
        if in_string {
            if escaped {
                ...
                in_string = false;
                let key = raw.get(string_start..index)?;
                if object_depth == 1 && key == "schemaVersion" {
                    key_span = Some((string_start.saturating_sub(1), index + 1));
                    break;
                }
            }
            continue;
    ...
    let (key_start, key_end) = key_span?;
    let after_key = raw.get(key_end..)?;
    let after_whitespace = after_key.trim_start();
    let colon = after_whitespace.strip_prefix(':')?;
    let value_tail = colon.trim_start();
    let value_start = raw.len().checked_sub(value_tail.len())?;
    let expected = expected_version.to_string();
    let remainder = value_tail.strip_prefix(&expected)?;
    if remainder
        .chars()
        .next()
    ...
    let value_end = value_start.checked_add(expected.len())?;
    let mut rewritten = String::with_capacity(raw.len() + discriminator.len());
    rewritten.push_str(raw.get(..key_start)?);
    rewritten.push_str("\"schema\"");
    rewritten.push_str(raw.get(key_end..value_start)?);
    rewritten.push('"');
    rewritten.push_str(discriminator);
    rewritten.push('"');
    rewritten.push_str(raw.get(value_end..)?);
    Some(rewritten)
}
...
mod tests {
    ...
    #[test]
    fn legacy_catalog_v1_remains_supported() {
        let cat =
            catalog_json(ASYM_KEY).replacen("\"schema\": \"catalog\"", "\"schemaVersion\": 1", 1);
        let pol = policy_json(READER_ROLE, "");
        let (catalog, _, _, _) = load(&cat, &pol).expect("catalog v1 remains accepted");
        assert_eq!(catalog.schema, crate::catalog::CatalogSchema::Catalog);
    }

    #[test]
    fn legacy_policy_v2_remains_supported() {
        let cat = catalog_json(ASYM_KEY);
        let pol = policy_json(READER_ROLE, "").replacen(
            "\"schema\": \"policy\"",
            "\"schemaVersion\": 2",
            1,
        );
        let (_, _, _, _) = load(&cat, &pol).expect("policy v2 remains accepted");
    }
