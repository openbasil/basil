...
use crate::core::crypto_provider::{
    CryptoProviderId, CustodyMode, ProviderMetadata, ProviderPolicy,
};

const MAX_TOKEN_BYTES: usize = 32 * 1024;
pub(super) const MAX_AUDIT_SHORT_BYTES: usize = 128;
pub(super) const MAX_AUDIT_TEXT_BYTES: usize = 256;
pub(super) const MAX_AUDIT_WORKFLOW_BYTES: usize = 512;
pub(super) const MAX_PROVIDER_URL_BYTES: usize = 512;
...
const MAX_ARTIFACT_SIGN_KEY_IDS: usize = 64;
const MAX_PROVIDER_RULES: usize = 64;
const MAX_CA_BUNDLE_BYTES: usize = 256 * 1024;
const MAX_CA_CERTIFICATES: usize = 64;
...
pub struct GithubActionsRule {
    ...
    /// Exact JWKS URL.
    pub jwks_url: String,
    /// Optional absolute path to the exclusive PEM trust roots for this rule.
    #[serde(default)]
...
/// A closed operation profile granted by a provider rule.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Deserialize, Serialize)]
#[serde(rename_all = "kebab-case")]
pub enum ProviderOperationProfile {
    /// Sign an artifact with one of the rule's exact target key IDs.
    ArtifactSign,
}
...
pub struct ProviderRule {
    ...
    pub operation_profiles: Vec<ProviderOperationProfile>,
    /// Exact key IDs that the `artifact-sign` profile may target.
    pub artifact_sign_key_ids: Vec<String>,
...
impl ProviderConfig {
    ...
    /// Optional exclusive PEM trust-root bundle for this provider.
    #[must_use]
    pub fn ca_bundle_path(&self) -> Option<&Path> {
        match self {
            Self::GithubActions(rule) => rule.ca_bundle_path.as_deref(),
            Self::ForgejoActions(rule) => rule.ca_bundle_path.as_deref(),
        }
    }
...
#[derive(Clone)]
struct ResolvedProviderTrust {
    custom_roots: Option<Vec<reqwest::Certificate>>,
    pinned_file: Option<(PathBuf, CaFileIdentity)>,
}

impl std::fmt::Debug for ResolvedProviderTrust {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("ResolvedProviderTrust")
            .field(
                "custom_root_count",
                &self.custom_roots.as_ref().map_or(0, Vec::len),
            )
            .field("has_pinned_file", &self.pinned_file.is_some())
            .finish()
    }
}

impl ResolvedProviderTrust {
    fn revalidate(&self) -> Result<(), FederationError> {
        let Some((path, pinned)) = &self.pinned_file else {
            return Ok(());
        };
        let (file, current) = open_validated_ca_file(path)?;
        drop(file);
        if &current != pinned {
            return Err(FederationError::InvalidCaBundle);
        }
        Ok(())
    }

    fn build_client(&self) -> Result<reqwest::Client, FederationError> {
        ...
        let mut builder = reqwest::Client::builder()
            ...
            .timeout(timeout);
        if let Some(certificates) = &self.custom_roots {
            builder = builder.tls_certs_only(certificates.clone());
        }
        builder
            .build()
            .map_err(|_| FederationError::InvalidCaBundle)
    }
}

fn resolve_provider_trust(
    provider: &ProviderConfig,
) -> Result<ResolvedProviderTrust, FederationError> {
    let Some(path) = provider.ca_bundle_path() else {
        return Ok(ResolvedProviderTrust {
            custom_roots: None,
            pinned_file: None,
        });
    };
    let (mut file, before) = open_validated_ca_file(path)?;
    if before.length == 0 || before.length > MAX_CA_BUNDLE_BYTES as u64 {
        return Err(FederationError::InvalidCaBundle);
    }
    let mut bytes = Zeroizing::new(Vec::new());
    file.by_ref()
        .take((MAX_CA_BUNDLE_BYTES + 1) as u64)
    ...
    let after = file
        .metadata()
        .map(|metadata| CaFileIdentity::from_metadata(&metadata))
        .map_err(|_| FederationError::InvalidCaBundle)?;
    if before != after || before.length != bytes.len() as u64 {
        return Err(FederationError::InvalidCaBundle);
    }
    let certificates = parse_certificate_only_pem(&bytes)?;
    Ok(ResolvedProviderTrust {
        custom_roots: Some(certificates),
        pinned_file: Some((path.to_path_buf(), before)),
    })
}

fn parse_certificate_only_pem(bytes: &[u8]) -> Result<Vec<reqwest::Certificate>, FederationError> {
    ...
    loop {
        ...
        let (der_rest, _certificate) = x509_parser::parse_x509_certificate(&pem.contents)
            .map_err(|_| FederationError::InvalidCaBundle)?;
        if !der_rest.is_empty() || certificates.len() >= MAX_CA_CERTIFICATES {
            return Err(FederationError::InvalidCaBundle);
        }
        certificates.push(
...
fn open_validated_ca_file(path: &Path) -> Result<(File, CaFileIdentity), FederationError> {
    let relative = path
        .strip_prefix("/")
        .map_err(|_| FederationError::InvalidCaBundle)?;
    ...
    for (index, component) in relative.components().enumerate() {
        ...
        if is_leaf {
            ...
            let metadata = file
                .metadata()
                .map_err(|_| FederationError::InvalidCaBundle)?;
            validate_ca_leaf(&metadata)?;
            return Ok((file, CaFileIdentity::from_metadata(&metadata)));
        }
        let next = File::from(
            rustix::fs::openat(
                &directory,
    ...
    Err(FederationError::InvalidCaBundle)
}

fn validate_ca_directory(metadata: &std::fs::Metadata) -> Result<(), FederationError> {
    let effective_uid = rustix::process::geteuid().as_raw();
    let mode = metadata.permissions().mode();
    let owner_is_safe = metadata.uid() == 0 || metadata.uid() == effective_uid;
    if !metadata.is_dir() || !owner_is_safe || mode & 0o022 != 0 {
        return Err(FederationError::InvalidCaBundle);
    }
    Ok(())
}

fn validate_ca_leaf(metadata: &std::fs::Metadata) -> Result<(), FederationError> {
    let effective_uid = rustix::process::geteuid().as_raw();
    if !metadata.is_file()
        || metadata.nlink() != 1
...
pub struct ProviderCatalog {
    ...
    resolved_trust: BTreeMap<String, ResolvedProviderTrust>,
}

impl Serialize for ProviderCatalog {
...
impl ProviderCatalog {
    ...
    pub fn with_experimental_providers(
        ...
        if rules.len() > MAX_PROVIDER_RULES {
            return Err(FederationError::Oversized("provider catalog"));
        }
        ...
        let resolved_trust = rules
            .iter()
            .map(|rule| {
                resolve_provider_trust(&rule.provider).map(|trust| (rule.id.clone(), trust))
            })
            .collect::<Result<_, _>>()?;
        Ok(Self {
            rules,
    ...
    /// Revalidate that every custom CA path still names the file pinned when
    /// the catalog was resolved.
    pub fn revalidate_trust(&self) -> Result<(), FederationError> {
        for trust in self.resolved_trust.values() {
            trust.revalidate()?;
        }
        Ok(())
    }

    /// Reject artifact targets whose signing path requires provider dispatch.
    ///
    /// Federated invocation v1 dispatches through the backend manager, so a
    /// software-custodied or ML-DSA target cannot be admitted without bypassing
    /// provider selection and its custody authorization.
    pub(crate) fn validate_artifact_targets(
        &self,
        catalog: &Catalog,
        ...
        for target in self
            .rules
            .iter()
            .flat_map(|rule| &rule.artifact_sign_key_ids)
        {
            let Some(key) = catalog.keys.get(target) else {
                continue;
            };
            let metadata = ProviderMetadata::from_key(key);
            if matches!(
                key.key_type,
                Some(KeyAlgorithm::MlDsa44 | KeyAlgorithm::MlDsa65 | KeyAlgorithm::MlDsa87)
            ) || metadata.provider == Some(CryptoProviderId::LocalSoftware)
                || metadata.custody == Some(CustodyMode::SoftwareEncrypted)
                || metadata.policy == ProviderPolicy::LocalSoftware
            {
                return Err(FederationError::UnsupportedArtifactSignTarget);
            }
        }
        Ok(())
    }

    /// Build one isolated HTTP client per rule from the catalog's pinned trust.
    pub(crate) fn build_generation_clients(
        &self,
    ) -> Result<BTreeMap<String, reqwest::Client>, FederationError> {
        self.revalidate_trust()?;
        self.rules
            .iter()
            .map(|rule| {
                let trust = self
                    .resolved_trust
                    .get(&rule.id)
                    .ok_or(FederationError::InvalidCaBundle)?;
                trust.build_client().map(|client| (rule.id.clone(), client))
            })
            .collect()
    }
...
impl TokenCorrelationKey {
    ...
    /// Compute the keyed token correlation used before identity verification.
    pub(crate) fn token_digest(&self, token: &[u8]) -> Result<[u8; 32], FederationError> {
        self.digest("basil-ci-token", token)
    }
}
...
pub struct VerifiedProviderEvidence {
    ...
    /// Exact issuer selected from trusted configuration.
    pub issuer: String,
    /// Stable configured rule identifier.
    pub rule_id: String,
    /// Policy subject granted by the rule.
    pub subject: String,
    /// Broker audience bound by the selected rule.
    pub broker_audience: String,
    /// Closed operation profiles granted by the selected rule.
    pub operation_profiles: Vec<ProviderOperationProfile>,
    /// Exact signing targets granted to the `artifact-sign` profile.
    pub artifact_sign_key_ids: Vec<String>,
...
/// Atomic result of one admitted per-run quota charge.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct RunQuotaReceipt {
    /// Bucket count after this charge.
    pub(crate) charged_count: u64,
    /// Operations remaining under the exact configured limit.
...
impl RunQuotaTable {
    ...
    pub fn charge(
        ...
        self.charge_with_receipt(generation, key, limit, retention_secs, now_unix)
            .map(|_| ())
    }

    /// Charge one operation and return the count computed under the table's
    /// caller-held mutex.
    ///
    /// # Errors
    ///
    /// Returns the same closed denial states as [`Self::charge`].
    pub(crate) fn charge_with_receipt(
        &mut self,
        generation: u64,
        key: &RunQuotaKey,
        limit: Option<u64>,
        retention_secs: u64,
        now_unix: i64,
    ) -> Result<RunQuotaReceipt, RunQuotaDenied> {
        ...
        if let Some(bucket) = self.counters.get_mut(key) {
            ...
            return Ok(RunQuotaReceipt {
                charged_count: bucket.count,
...
impl GenerationJwksCache {
    ...
    pub fn install(&mut self, keys: GenerationJwks, now: SystemTime) {
        if self.fetched_at.is_some_and(|installed| now < installed) {
            return;
        }
        self.keys = Some(keys);
        self.fetched_at = Some(now);
    ...
    fn fetch_and_parse<F: ProviderDocumentFetcher>(
        ...
        validate_discovery_document(&discovery.body, &self.issuer, self.jwks_url.as_str())?;
...
pub enum FederationError {
    ...
    ProviderRejected,
    /// An artifact target requires a signing dispatch unavailable to federation v1.
    #[error("unsupported federated artifact-sign target")]
    UnsupportedArtifactSignTarget,
    ...
    InvalidUrl,
    /// A configured custom trust-root bundle failed closed.
    #[error("invalid provider CA bundle")]
    InvalidCaBundle,
...
fn validate_common_rule(rule: &ProviderRule) -> Result<(), FederationError> {
    if rule.id.is_empty()
        ...
        || rule.artifact_sign_key_ids.is_empty()
        || rule.artifact_sign_key_ids.len() > MAX_ARTIFACT_SIGN_KEY_IDS
    ...
    for value in [&rule.id, &rule.subject, &rule.audience] {
    ...
    if !profiles.contains(&ProviderOperationProfile::ArtifactSign) {
        return Err(FederationError::ProviderRejected);
    }
    let mut target_ids = std::collections::BTreeSet::new();
    for key_id in &rule.artifact_sign_key_ids {
        if basil_cose::KeyId::from_text(key_id).is_err()
            || key_id.chars().any(char::is_control)
            || !target_ids.insert(key_id)
        {
            return Err(FederationError::ProviderRejected);
        }
    }
...
/// Select exactly one bounded provider token before any JWT parsing or
/// correlation work.
pub(crate) fn exactly_one_bounded_token(tokens: &[String]) -> Result<&str, FederationError> {
...
fn validate_audit_claim(value: &str, max_bytes: usize) -> Result<(), FederationError> {
...
fn validate_github_audit_claims(claims: &GithubClaims) -> Result<(), FederationError> {
    for (value, bound) in [
        (claims.iss.as_str(), MAX_PROVIDER_URL_BYTES),
        (claims.aud.as_str(), MAX_AUDIT_WORKFLOW_BYTES),
        (claims.sub.as_str(), MAX_AUDIT_WORKFLOW_BYTES),
        (claims.repository.as_str(), MAX_AUDIT_TEXT_BYTES),
        (claims.event_name.as_str(), MAX_AUDIT_SHORT_BYTES),
        (claims.ref_field.as_str(), MAX_AUDIT_TEXT_BYTES),
        (claims.job_workflow_ref.as_str(), MAX_AUDIT_WORKFLOW_BYTES),
        (claims.job_workflow_sha.as_str(), MAX_AUDIT_SHORT_BYTES),
        (claims.runner_environment.as_str(), MAX_AUDIT_SHORT_BYTES),
        (claims.jti.as_str(), MAX_AUDIT_WORKFLOW_BYTES),
    ] {
        validate_audit_claim(value, bound)?;
    }
    if let Some(environment) = claims.environment.as_deref() {
        validate_audit_claim(environment, MAX_AUDIT_TEXT_BYTES)?;
    }
    Ok(())
}

fn validate_forgejo_audit_claims(claims: &ForgejoClaims) -> Result<(), FederationError> {
    for (value, bound) in [
        (claims.iss.as_str(), MAX_PROVIDER_URL_BYTES),
        (claims.aud.as_str(), MAX_AUDIT_WORKFLOW_BYTES),
        (claims.sub.as_str(), MAX_AUDIT_WORKFLOW_BYTES),
        ...
        (claims.event_name.as_str(), MAX_AUDIT_SHORT_BYTES),
        (claims.ref_field.as_str(), MAX_AUDIT_TEXT_BYTES),
        (claims.ref_type.as_str(), MAX_AUDIT_SHORT_BYTES),
        (claims.sha.as_str(), MAX_AUDIT_SHORT_BYTES),
        (claims.workflow_ref.as_str(), MAX_AUDIT_WORKFLOW_BYTES),
        (claims.jti.as_str(), MAX_AUDIT_WORKFLOW_BYTES),
    ] {
        validate_audit_claim(value, bound)?;
    }
...
pub fn verify_github(
    ...
    validate_github_audit_claims(&claims)?;
    let now_secs = now
    ...
    let token_digest = correlation.token_digest(token.as_bytes())?;
    Ok(GithubEvidence {
...
pub fn verify_forgejo(
    ...
    let claims: ForgejoClaims =
        serde_json::from_value(data.claims).map_err(|_| FederationError::TokenRejected)?;
    validate_forgejo_audit_claims(&claims)?;
    let now_secs = now
        .duration_since(UNIX_EPOCH)
        .map_err(|_| FederationError::TokenRejected)?
        .as_secs();
...
pub async fn fetch_generation_jwks(
    ...
    validate_discovery_document(&discovery_body, provider.issuer(), provider.jwks_url())?;
    let jwks = client
        .get(jwks_url.clone())
        .send()
        .await
...
fn validate_discovery_document(
    ...
    let document: DiscoveryDocument =
        serde_json::from_slice(body).map_err(|_| FederationError::Malformed)?;
    if document.issuer != expected_issuer || document.jwks_uri != expected_jwks_uri {
        return Err(FederationError::ProviderRejected);
    }
    Ok(())
}
...
fn validate_urls(issuer: &str, discovery: &str, jwks: &str) -> Result<(), FederationError> {
    if [issuer, discovery, jwks]
        .iter()
        .any(|value| value.len() > MAX_PROVIDER_URL_BYTES)
    {
        return Err(FederationError::InvalidUrl);
    }
...
mod tests {
    ...
    impl TestDirectory {
        fn create() -> Self {
            ...
            let path = parent.join(format!("basil-ci-federation-ca-{}", uuid::Uuid::new_v4()));
            std::fs::create_dir(&path).expect("create temporary CA directory");
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o700))
                .expect("protect temporary CA directory");
            Self(path)
        }
    ...
    fn github_rule_with_ca(id: &str, path: PathBuf) -> ProviderRule {
        let mut rule = github_rule(id, &["refs/heads/main"], &["push"]);
        if let ProviderConfig::GithubActions(provider) = &mut rule.provider {
            provider.ca_bundle_path = Some(path);
        }
        rule
    }
    ...
    #[test]
    fn discovery_requires_exact_unique_issuer_and_jwks_uri() {
        let provider = cache_rule();
        let exact = format!(
            r#"{{"issuer":"{}","jwks_uri":"{}","other":true}}"#,
            provider.issuer(),
            provider.jwks_url()
        );
        assert!(
            validate_discovery_document(exact.as_bytes(), provider.issuer(), provider.jwks_url())
                .is_ok()
        );
        let wrong_issuer = format!(
            r#"{{"issuer":"https://retired.example","jwks_uri":"{}"}}"#,
            provider.jwks_url()
        );
        assert_eq!(
            validate_discovery_document(
                wrong_issuer.as_bytes(),
                provider.issuer(),
                provider.jwks_url()
            ),
        ...
        for malformed in [
            r#"{"jwks_uri":"https://example.test/jwks"}"#,
            r#"{"issuer":"https://example.test"}"#,
            r#"{"issuer":7,"jwks_uri":"https://example.test/jwks"}"#,
            r#"{"issuer":"https://example.test","jwks_uri":7}"#,
        ] {
            assert_eq!(
                validate_discovery_document(
                    malformed.as_bytes(),
    ...
    #[test]
    fn catalog_pins_validated_custom_ca_identity() {
        use std::fs;
        use std::io::Write as _;
        use std::os::unix::fs::PermissionsExt as _;

        let fixture = TestDirectory::create();
        let path = fixture.path().join("provider-ca.pem");
        fs::write(&path, include_bytes!("../../testdata/jwks_tls_cert.pem"))
            .expect("write CA bundle");
        fs::set_permissions(&path, fs::Permissions::from_mode(0o600)).expect("protect CA bundle");
        let rule = github_rule_with_ca("custom-ca", path.clone());
        let catalog = ProviderCatalog::new(vec![rule]).expect("resolve custom CA once");
        assert_eq!(
            catalog
                .build_generation_clients()
                .expect("build client")
                .len(),
            1
        );

        let replacement = fixture.path().join("replacement.pem");
        fs::write(
            &replacement,
            include_bytes!("../../testdata/jwks_tls_cert.pem"),
        )
        .expect("write replacement CA");
        fs::set_permissions(&replacement, fs::Permissions::from_mode(0o600))
            .expect("protect replacement CA");
        fs::rename(&replacement, &path).expect("replace pinned CA pathname");
        assert_eq!(
            catalog.revalidate_trust(),
            Err(FederationError::InvalidCaBundle)
        );

        let in_place = fixture.path().join("in-place.pem");
        fs::write(
            &in_place,
            include_bytes!("../../testdata/jwks_tls_cert.pem"),
        )
        .expect("write in-place CA");
        fs::set_permissions(&in_place, fs::Permissions::from_mode(0o600))
            .expect("protect in-place CA");
        let rule = github_rule_with_ca("in-place", in_place.clone());
        let catalog = ProviderCatalog::new(vec![rule]).expect("pin in-place CA");
        fs::OpenOptions::new()
            .append(true)
            .open(&in_place)
            .and_then(|mut file| file.write_all(b"\n"))
            .expect("rewrite CA through the same inode");
        assert_eq!(
            catalog.revalidate_trust(),
            Err(FederationError::InvalidCaBundle)
        );
    ...
    fn catalog_rejects_unsafe_or_malformed_custom_ca_paths() {
        use std::fs;
        use std::os::unix::fs::{PermissionsExt as _, symlink};

        let fixture = TestDirectory::create();
        let valid = fixture.path().join("valid.pem");
        fs::write(&valid, include_bytes!("../../testdata/jwks_tls_cert.pem"))
            .expect("write CA bundle");
        fs::set_permissions(&valid, fs::Permissions::from_mode(0o600)).expect("protect CA bundle");
        let case = |path: PathBuf| {
            let rule = github_rule_with_ca("custom-ca", path);
            ProviderCatalog::new(vec![rule])
        };

        let relative = PathBuf::from("relative.pem");
        assert!(matches!(
            case(relative),
            Err(FederationError::InvalidCaBundle)
        ));
        let link = fixture.path().join("link.pem");
        symlink(&valid, &link).expect("create symlink");
        assert!(matches!(case(link), Err(FederationError::InvalidCaBundle)));
        let malformed = fixture.path().join("malformed.pem");
        fs::write(&malformed, b"not a certificate").expect("write malformed PEM");
        fs::set_permissions(&malformed, fs::Permissions::from_mode(0o600))
            .expect("protect malformed PEM");
        assert!(matches!(
            case(malformed),
            Err(FederationError::InvalidCaBundle)
        ));
        fs::set_permissions(&valid, fs::Permissions::from_mode(0o622)).expect("loosen CA bundle");
        assert!(matches!(case(valid), Err(FederationError::InvalidCaBundle)));

        let empty = fixture.path().join("empty.pem");
        fs::write(&empty, []).expect("write empty bundle");
        fs::set_permissions(&empty, fs::Permissions::from_mode(0o600))
            .expect("protect empty bundle");
        assert!(matches!(case(empty), Err(FederationError::InvalidCaBundle)));
        let oversized = fixture.path().join("oversized.pem");
        let oversized_file = fs::File::create(&oversized).expect("create oversized bundle");
        oversized_file
            .set_len((MAX_CA_BUNDLE_BYTES + 1) as u64)
            .expect("size oversized bundle");
        drop(oversized_file);
        fs::set_permissions(&oversized, fs::Permissions::from_mode(0o600))
            .expect("protect oversized bundle");
        assert!(matches!(
            case(oversized),
            Err(FederationError::InvalidCaBundle)
        ));
        let too_many_certificates = fixture.path().join("too-many-certificates.pem");
        fs::write(
            &too_many_certificates,
            include_bytes!("../../testdata/jwks_tls_cert.pem").repeat(MAX_CA_CERTIFICATES + 1),
        )
        .expect("write excessive certificate bundle");
        fs::set_permissions(&too_many_certificates, fs::Permissions::from_mode(0o600))
            .expect("protect excessive certificate bundle");
        assert!(matches!(
            case(too_many_certificates),
            Err(FederationError::InvalidCaBundle)
        ));

        let linked_source = fixture.path().join("linked-source.pem");
        let linked_leaf = fixture.path().join("linked-leaf.pem");
        fs::write(
            &linked_source,
            include_bytes!("../../testdata/jwks_tls_cert.pem"),
        )
        .expect("write linked bundle");
        fs::set_permissions(&linked_source, fs::Permissions::from_mode(0o600))
            .expect("protect linked bundle");
        fs::hard_link(&linked_source, &linked_leaf).expect("hard-link bundle");
        assert!(matches!(
            case(linked_leaf),
            Err(FederationError::InvalidCaBundle)
        ));

        let safe_directory = fixture.path().join("safe-directory");
        fs::create_dir(&safe_directory).expect("create safe directory");
        fs::set_permissions(&safe_directory, fs::Permissions::from_mode(0o700))
            .expect("protect safe directory");
        let nested_ca = safe_directory.join("ca.pem");
        fs::write(
            &nested_ca,
            include_bytes!("../../testdata/jwks_tls_cert.pem"),
        )
        .expect("write nested CA");
        fs::set_permissions(&nested_ca, fs::Permissions::from_mode(0o600))
            .expect("protect nested CA");
        let linked_directory = fixture.path().join("linked-directory");
        symlink(&safe_directory, &linked_directory).expect("symlink ancestor");
        assert!(matches!(
            case(linked_directory.join("ca.pem")),
            Err(FederationError::InvalidCaBundle)
        ));

        let writable_directory = fixture.path().join("writable-directory");
        fs::create_dir(&writable_directory).expect("create writable directory");
        fs::set_permissions(&writable_directory, fs::Permissions::from_mode(0o777))
            .expect("make ancestor writable");
        let writable_nested_ca = writable_directory.join("ca.pem");
        fs::write(
            &writable_nested_ca,
            include_bytes!("../../testdata/jwks_tls_cert.pem"),
        )
        .expect("write CA below writable ancestor");
        fs::set_permissions(&writable_nested_ca, fs::Permissions::from_mode(0o600))
            .expect("protect nested CA leaf");
        assert!(matches!(
            case(writable_nested_ca),
            Err(FederationError::InvalidCaBundle)
        ));
    }

    #[test]
    fn ca_parser_accepts_only_certificate_pem_sections() {
        let certificate = include_bytes!("../../testdata/jwks_tls_cert.pem");
        assert_eq!(
            parse_certificate_only_pem(certificate).map(|certificates| certificates.len()),
            Ok(1)
        );

        let mut mixed = Zeroizing::new(certificate.to_vec());
        mixed.extend_from_slice(include_bytes!("../../testdata/jwks_tls_key.pem"));
        assert!(matches!(
            parse_certificate_only_pem(&mixed),
            Err(FederationError::InvalidCaBundle)
        ));

        let mut prefixed = b"not PEM\n".to_vec();
        prefixed.extend_from_slice(certificate);
        assert!(matches!(
            parse_certificate_only_pem(&prefixed),
            Err(FederationError::InvalidCaBundle)
        ));

        let mut suffixed = certificate.to_vec();
        suffixed.extend_from_slice(b"not PEM\n");
        assert!(matches!(
            parse_certificate_only_pem(&suffixed),
            Err(FederationError::InvalidCaBundle)
        ));
    }
    ...
    #[test]
    fn provider_token_shape_and_bound_precede_jwt_parsing() {
        assert_eq!(
            exactly_one_bounded_token(&[]),
            Err(FederationError::Malformed)
        );
        assert_eq!(
            exactly_one_bounded_token(&["one".to_string(), "two".to_string()]),
            Err(FederationError::Malformed)
        );
        assert_eq!(
            exactly_one_bounded_token(&["x".repeat(MAX_TOKEN_BYTES + 1)]),
            Err(FederationError::Oversized("token"))
        );
        assert_eq!(
            exactly_one_bounded_token(&["not-a-jwt".to_string()]),
            Ok("not-a-jwt")
        );
    }

    #[test]
    fn audit_projected_claims_reject_controls_and_oversize() {
        let github = |repository: String| GithubClaims {
            iss: GITHUB_ISSUER.to_string(),
            aud: "urn:basil:ci:jkt:proof".to_string(),
            sub: "repo:openbasil/basil".to_string(),
            repository,
            repository_id: "42".to_string(),
            repository_owner_id: "7".to_string(),
            actor_id: Some("9".to_string()),
        ...
        assert_eq!(
            validate_github_audit_claims(&github("repo\nname".to_string())),
            Err(FederationError::ProviderRejected)
        );
        assert_eq!(
            validate_github_audit_claims(&github("x".repeat(MAX_AUDIT_TEXT_BYTES + 1))),
            Err(FederationError::ProviderRejected)
        );

        let forgejo = ForgejoClaims {
            iss: "https://forge.example/api/actions".to_string(),
            aud: "urn:basil:ci:jkt:proof".to_string(),
            sub: "repo:forge/basil".to_string(),
            repository: "forge/basil".to_string(),
            repository_id: "11".to_string(),
            repository_owner_id: "3".to_string(),
            actor_id: None,
            event_name: "push".to_string(),
            ref_field: "refs/heads/main".to_string(),
            ref_type: "branch".to_string(),
            sha: "b".repeat(40),
            workflow_ref: "forge/basil/.forgejo/workflows/release.yml@refs/heads/main\u{7f}"
                .to_string(),
            run_id: "900".to_string(),
            run_attempt: "1".to_string(),
            environment: None,
        ...
        assert_eq!(
            validate_forgejo_audit_claims(&forgejo),
            Err(FederationError::ProviderRejected)
        );
    }
    ...
    #[test]
    fn older_refresh_completion_cannot_replace_newer_keys() {
        let rule = cache_rule();
        let mut cache =
            GenerationJwksCache::new(1, &rule, JwksCachePolicy::default()).expect("cache");
        let newer = GenerationJwks::parse(1, &jwks("newer").body).expect("newer JWKS");
        let older = GenerationJwks::parse(1, &jwks("older").body).expect("older JWKS");
        let earlier = UNIX_EPOCH + Duration::from_secs(10);
        let later = UNIX_EPOCH + Duration::from_secs(20);

        cache.install(newer, later);
        cache.install(older, earlier);

        assert!(cache.cached_key("newer").is_some());
        assert!(cache.cached_key("older").is_none());
    }
    ...
    #[test]
    fn catalog_requires_a_closed_profile_and_exact_artifact_targets() {
        let mut empty_profiles = github_rule("release", &["main"], &["push"]);
        empty_profiles.operation_profiles.clear();
        assert!(matches!(
            ProviderCatalog::new(vec![empty_profiles]),
            Err(FederationError::ProviderRejected)
        ));

        let mut empty_targets = github_rule("release", &["main"], &["push"]);
        empty_targets.artifact_sign_key_ids.clear();
        assert!(matches!(
            ProviderCatalog::new(vec![empty_targets]),
            Err(FederationError::ProviderRejected)
        ));

        let mut duplicate_targets = github_rule("release", &["main"], &["push"]);
        duplicate_targets
            .artifact_sign_key_ids
            .push("release-signing".to_string());
        assert!(matches!(
            ProviderCatalog::new(vec![duplicate_targets]),
            Err(FederationError::ProviderRejected)
        ));

        let mut oversized_target = github_rule("release", &["main"], &["push"]);
        oversized_target.artifact_sign_key_ids = vec!["k".repeat(129)];
        assert!(matches!(
            ProviderCatalog::new(vec![oversized_target]),
            Err(FederationError::ProviderRejected)
        ));
    }

    #[test]
    fn provider_rule_schema_rejects_unknown_profiles_and_missing_target_scope() {
        let rule = github_rule("release", &["main"], &["push"]);
        let mut unknown = serde_json::to_value(&rule).expect("rule serializes");
        unknown["operationProfiles"] = json!(["raw-sign"]);
        let error = serde_json::from_value::<ProviderRule>(unknown)
            .expect_err("unknown operation profile must not deserialize");
        assert!(error.to_string().contains("unknown variant `raw-sign`"));

        let mut missing_targets = serde_json::to_value(rule).expect("rule serializes");
        missing_targets
            .as_object_mut()
            .expect("rule is an object")
            .remove("artifactSignKeyIds");
        let error = serde_json::from_value::<ProviderRule>(missing_targets)
            .expect_err("missing target scope must not deserialize");
        assert!(error.to_string().contains("artifactSignKeyIds"));
    }
    ...
    #[test]
    fn run_quota_receipt_is_atomic_with_the_charge() {
        let mut table = RunQuotaTable::new(16);
        let key = quota_key("release", 901, 1);
        assert_eq!(
            table
                .charge_with_receipt(1, &key, Some(3), RETENTION, NOW)
                .expect("first charge"),
            RunQuotaReceipt {
                charged_count: 1,
                remaining: 2,
