...
impl BackendManager {
    ...
    async fn generate_missing(&self, name: &str, entry: &KeyEntry) -> Result<(), ReconcileError> {
        ...
        // The validated loader rejects this shape. Keep the mutation boundary
        // independently fail-closed so a programmatically constructed catalog
        // can never bypass the dedicated Nix cache enrollment operation and its
        // bootstrap-lockdown checks through startup reconcile.
        if entry.nix_cache.is_some() {
            return Err(to_err(ManagerError::Backend(BackendError::Backend(
                "a nixCache identity cannot be generated by startup reconcile; use explicit enrollment"
                    .to_string(),
            ))));
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn nix_cache_identity_never_auto_provisions_during_reconcile() {
        // Deliberately bypass `catalog::load` and deserialize the typed schema
        // directly. The loader rejects this shape, while this regression proves
        // the startup mutation boundary still makes no backend create call if an
        // embedding application constructs an unvalidated catalog.
        const NIX_GENERATE: &str = r#"{
          "schema": "catalog",
          "backends": { "b": { "kind": "vault", "addr": "http://127.0.0.1:8200" } },
          "keys": {
            "cache.signer": {
              ...
              "missing": "generate",
              "nixCache": {
                "keyName": "cache.example.org-1", "state": "pending", "backendVersion": 1
              },
              "description": "Nix cache signer"
            }
          }
        }"#;
        let catalog: Catalog = serde_json::from_str(NIX_GENERATE).expect("typed schema parses");
        let (backend, recorder) = MockBackend::new(Probe::Absent);
        let mut backends: BTreeMap<String, Box<dyn Backend>> = BTreeMap::new();
        backends.insert("b".to_string(), Box::new(backend));
        let manager = BackendManager::new(catalog, backends).expect("manager constructs");

        let error = manager
            .reconcile()
            .await
            .expect_err("Nix cache startup generation must fail closed");
        assert!(matches!(error, ReconcileError::Generate { .. }));
        assert_eq!(
            recorder.create_named_key_calls.load(Ordering::SeqCst),
            0,
            "startup reconcile must not call create_named_key for nixCache"
        );
    }
