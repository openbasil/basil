...
- **Reload isolation and policy separation**: `ci_reload_policy_matrix` drives three live
  ...
  the subject lane keeps serving). `ci_trust_reload_matrix` covers the provider-trust half
  of the reload boundary with a loopback HTTPS origin: it rotates the CA, server
  certificate, and JWKS key while issuer, URLs, and `kid` remain identical. Generation B
  accepts only the new trust, generation A remains pinned, wrong and retired roots fail
  closed, discovery redirects and identity mismatches are rejected, and an outage serves
  cached keys only through the configured stale window.
- **CI qualification evidence contract**: `ci_qualification_evidence_fixture` verifies a
  bounded version 1 inventory and its exact checked-in client and broker-audit record bytes
  without fetching anything. The adversarial cases reject closed-schema violations,
  duplicate or missing records, unsafe paths and encodings, symlink escapes, unsupported
  versions, multiply linked records, byte-integrity failures, and cross-record redirection.
  The fixture is marked `fixture_only` and `not-qualified`: it proves the offline schema
  mechanics, not that GitHub or Forgejo ran a qualification workflow.
