...
/// Ordinary rows use a fresh courier source per issuance, while the
/// per-source row explicitly reuses stable A/B sources. This keeps the
/// burst-only source bucket local to the row that exercises it.
...
/// Process-wide uniqueness for automatically generated courier sources.
...
fn challenge_lifecycle_and_capacity_over_the_real_rpc() {
    ...
    runtime.block_on(async {
        ...
        // Restart intentionally between the reload and capacity rows. This
        // gives the global-capacity oracle a known-empty challenge table and
        // fresh global/JKT/source rate state; reconnect because channels to
        // the stopped agent are stale.
        ...
        // full).
        let pre_restart = global_capacity_row(&mut client, &ctx, jkt_a).await;
...
async fn global_capacity_row(
    ...
    assert_eq!(
        ...
        "the intentional pre-capacity restart resets the serving generation"
    );
    assert_eq!(
        pre_restart.generation, outstanding.generation,
        "both controls ride the fresh agent generation"
    );

    // The restart makes the accounting exact: two controls occupy slots, so
    // exactly `capacity - 2` fills must succeed. Fill JKT partitions remain
    // at or below eight outstanding entries, and automatically generated
    // courier sources keep per-source rate limiting out of this row.
    let expected_fills = CHALLENGE_SHAPE
        .capacity
        .checked_sub(2)
        .expect("capacity accommodates the two controls");
    for filled in 0..expected_fills {
        let partition = u8::try_from(filled / PER_JKT_OUTSTANDING_CAP)
            .expect("capacity fill partitions fit in a byte");
        let jkt_fill = fill_jkt(0xC0, partition);
        issue(client, jkt_fill, None)
            .await
            .unwrap_or_else(|status| {
                panic!(
                    "global fill {filled} of {expected_fills} must succeed after a fresh restart, got {status:?}"
                )
            });
    }
    let declined = issue(client, fill_jkt(0xC1, 0), None)
        .await
        .expect_err("issuance immediately beyond exact global capacity must decline");
    ...
    eprintln!("ROW global-capacity: ok ({expected_fills} exact fills to decline)");
    pre_restart
}
...
async fn restart_row(
    ...
    // This second restart clears the table that had just reached capacity
    // before the pressure-control consumption. The restarted broker issues
    // immediately, stamps a FRESH instance-ID prefix, and is back at
    // generation 1.
...
async fn issue(
    ...
    let courier_observed_source = Some(courier_observed_source.unwrap_or_else(|| {
        let counter = SOURCE_COUNTER.fetch_add(1, Ordering::Relaxed);
        format!("matrix-auto-source-{counter}")
    }));
    let response = client
        .get_invocation_challenge(GetInvocationChallengeRequest {
            jkt: jkt.to_vec(),
