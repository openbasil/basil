...
EOF
  ...
  log "writing pins file"
  {
    ...
    # Split the tag so `REUSE` does not parse this generator string as a second
    # source-file annotation with the shell escape and quote in its expression.
    printf '# SPDX-License-%s: Apache-2.0\n' 'Identifier'
